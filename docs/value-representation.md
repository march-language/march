...
comment governs — if this page and the code ever disagree, the code comment
...
restated as the one pure size formula every allocation site computes from; the
...
shares this 16-byte prefix on purpose, so string values can be probed for
...
- **`emit_untag_scalar`** — **conditional** untag reading *out of* a generic
  slot that may hold either a tagged scalar or a genuine heap pointer flowing
  through a scalar-typed view (e.g. dynamically-typed record/alist code whose
  static type lies about the runtime value): `and 1` to test the low bit,
...
- **NATURAL** (concrete/monomorphic slot): record fields and ADT fields whose
  type is statically known at the allocation site. Scalar values here are
...
≥ 4096 is bit-identical to a plausible heap address; only the static field
...
(`llvm_calls.ml`) exists at all: a closure that merely wraps a named
...
that comment and this section ever disagree, the comment wins).
...
(`fnv1a_64`, mirrored byte-for-byte against the C runtime's hash), then
...
`ensure_adt_eq_fn` (the "repr-audit" machinery, `MARCH_REPR_AUDIT=1`, added in
`f0fe40cc`).
...
`None` costs nothing (no allocation, a null check) and `Some` costs nothing
...
`and`/`icmp`/`select` — the niche machinery already proved this slot holds a
...
bumped, since the new record now holds an independent reference to the same
...
deliberate: NATURAL representation is ambiguous for a type-erased value (an
even `Int` ≥ 4096 is bit-identical to a plausible heap pointer, so a
plausible-heap sniff would wrongly `incrc`/dereference the integer — this is
precisely the `record_put`-large-even-int regression, §9).
...
whose value argument is tagged (kind byte `105 = 'i'`):
...
One line per bug whose root cause is a violation of a rule stated above,
...
| **`record_put` large-even-int SIGSEGV** — a NATURAL-repr even `Int` ≥ 4096 handed to `record_put` was sniffed as a heap pointer and dereferenced | §3/§8: type-erased record field writes must use UNIFORM (tagged) representation, never a "plausible heap pointer" runtime sniff over NATURAL bits | `b439d0f0` |
