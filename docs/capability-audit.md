...
nothing has changed it is quiet:
...
When a dependency asks for more than it used to, it says so and **exits 1**:
...
than it deserves is worse than no report, because it converts an open question
into a settled one.

### It proves

- **A change in declared authority is surfaced, transitively.** Every
  dependency, including dependencies of dependencies, is walked and compared.
...
- **Capability-passing code cannot under-declare.** Where a capability is
  ...
  declaration is a genuine lower bound for that code.
...
- **That a dependency declared everything it uses.** A module calling a
  ...
  audit` alone will not catch it. Treat the declared set as a floor that
  capability-passing code cannot go under, not as a ceiling on everything.
...
runtime symbols surviving dead-strip, and an embedded manifest when present. The
