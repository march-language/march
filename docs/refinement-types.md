...
fixture set built from known holes (a refinement two layers down a type
argument, `List(List({Int | _ > 0}))`; a fixture leaves the set once its
position is enforced, as eleven of them did on 2026-09-13, the arrow-domain
one last, when the audit learned that a single-argument callback's domain
is checked), pinned at
