...
- **Allowed if you prove it's safe**: partial functions that come with a refinement precondition, such as `List.nth`, `List.tail`, `List.head`, `Option.unwrap`, `Result.unwrap`, `Random.choice_weighted`, `Stats.percentile`, and `Array.get`/`Array.set`/`Array.pop` (index in bounds; `pop` on a non-empty array). Guard the call with something that proves the precondition, and it's accepted:
