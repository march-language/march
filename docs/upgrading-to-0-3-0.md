...
## 1. The capability ceiling is on by default
...
invisible to `forge check`/`forge build` and can carry capability-ceiling
...
`Vault(v)`, phantom in the type of the values it holds — the element type is
...
pointing at a `Vault.set`/`Vault.get` call, often nowhere near where the
...
`Ctor` — "stringified on the way in." That holds under the interpreter, but
...
upstream compiler repo's `specs/todos/` for the filed report.
...
## 6. `List.nth` now carries a bounds contract
...
Nothing is required — this is silent-by-default and every package surveyed
...
- **An entry file whose name does not match the package name.** `forge` derives
  ...
  carried a latent mismatch that only surfaced while upgrading, and the failure
...
1. Run `forge check` (or `forge build`) and work through the
   capability-ceiling errors; run `forge fix` first, it handles most of the
