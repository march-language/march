...
core. Any producer faster than that grows an unbounded mailbox forever. The first
...
- **`3` — `block_sender`** makes backpressure **transitive**: a full mailbox parks the
  ...
  which then naturally stops reading and lets TCP push back on the client. Nothing is
  lost. Use this on internal pipelines where every message matters, and always when the
  actor answers `Actor.call` (a dropped call request is indistinguishable from a lost
  reply).
...
the scheduler nothing), and replies are correlation-checked — a reply that arrives after
...
restart in lockstep. The streak resets once a child survives a full budget window.
