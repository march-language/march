...

{
  ...
  printf '> **Peak RSS and peak-live figures are currently contaminated by two compiled-only RC leaks**\n'
  printf '> found by this harness: `x ++ "literal"` leaks a fresh copy of the literal per evaluation,\n'
  printf '> and a hand-written recursive walk over a `List(String)` never frees the elements\n'
  printf '> (`string_split_large`: 9,000,126 string allocations, 3 frees). Memory numbers therefore\n'
  printf '> measure leaked memory as much as legitimate working set, and **must not be used to justify\n'
  printf '> a representation change until those leaks are fixed** — the fix is an RC correction, not a\n'
  printf '> new string representation. Timing, allocation counts and copy volumes are unaffected.\n\n'
