...
## Latest run: 2026-09-25, Linux x86_64

**Machine:** 4-vCPU cloud VM, `Intel(R) Xeon(R) Processor @ 2.10GHz`, 16 GB, Linux 6.18 x86_64,
load 0.78 at start, CPU steal under 1%. A shared-hypervisor VM, not a dedicated box: spreads are
wider than on the 2026-08-04 dedicated Xeon 8581C run, and a few (called out below) are wide
enough that the median should be read together with the min.
**Compiler:** `march 0.4.0` from the `nightly-20260924` release (commit `59ecf17`), run through the
new `MARCH=` override because this box had no opam switch. The seven benchmark sources are
byte-identical between `59ecf17` and the commit that recorded these numbers.
**Methodology:** `RUNS=10 MACHINE_OUT=… MARCH=… bash bench/run_benchmarks.sh`; median, min, max.
**Raw data:** [`results/2026-09-25-x86_64-xeon-cloud.txt`](results/2026-09-25-x86_64-xeon-cloud.txt)
(terminal output with the toolchain banner), [`.jsonl`](results/2026-09-25-x86_64-xeon-cloud.jsonl)
(machine-readable), [`.svg`](results/2026-09-25-x86_64-xeon-cloud.svg) (chart, generated by
`bench/chart.py`).

| Language | Version |
|----------|---------|
| March    | 0.4.0, nightly-20260924 (`--compile --opt 2`, clang 18) |
| OCaml    | 5.3.0 (`ocamlopt`) |
| Rust     | 1.94.1 (`rustc -O`) |
| Elixir   | 1.14.0 on Erlang/OTP 25 (script mode, BEAM JIT) |
| Python   | 3.11.15 (CPython) |
| NumPy    | 2.4.6 |

| Benchmark (median ms) | March | OCaml | Rust | Elixir | Python | NumPy |
|------------------------|------:|------:|-----:|-------:|-------:|------:|
| fib(40)                | 499.0 | 603.2 | **329.1** | 1837.3 | | |
| binary-trees(15)       | 414.9 | **31.3** | 142.1 | 490.5 | | |
| tree-transform         | **2844.2** | 7824.3 | 4983.7 | 5617.9 | | |
| list-ops(1M)           | 161.2 | 56.6 | **2.3** | 487.8 | | |
| simd-sum(5M)           | 5.6 | 6.9 | 5.5 | 195.3 | 258.0 | **3.3** |
| simd-map(5M)           | 19.5 | **9.1** | 19.6 | 1184.6 | 212.1 | 11.5 |
| simd-map2(5M)          | 97.4 | **11.0** | 20.1 | 655.3 | 218.8 | 11.8 |

Bold is the fastest row entry. NumPy-f32 (float32 input, not a like-for-like row) measured 1.7, 5.1
and 4.2 ms on the three SIMD benchmarks.

**Wide spreads in this run.** March tree-transform ranged 2157.6 to 3214.1 ms and the Elixir rows
swung by up to 3x; both are consistent with a noisy VM. **March simd-map2 is different:** it ranged
20.7 to 196.8 ms and is bimodal, not noisy. Repeated runs alternate between about 22 ms and about
160 ms, while Rust on the same box stays at 20 to 22 ms. It persists with one CPU, one scheduler,
the main thread pinned, and ASLR off; page-fault counts are identical in fast and slow runs. The
inlined map2 loop does fire in the emitted IR. Filed as
`specs/todos/2026-09-25-simd-map2-bimodal-timing.md`.

### Against the previous x86 run (2026-08-04, dedicated Xeon 8581C)

Absolute milliseconds from two different machines do not compare (see `specs/benchmarks.md`,
"why absolute-ms baselines cannot detect regressions"). The March-to-competitor ratio on the same
box does:

| Benchmark | March / Rust, Aug | March / Rust, Sep | March / OCaml, Aug | March / OCaml, Sep |
|-----------|------:|------:|------:|------:|
| fib(40)          | 1.60 | 1.52 | 0.77 | 0.83 |
| binary-trees(15) | 4.39 | **2.92** | 20.13 | **13.26** |
| tree-transform   | 0.63 | 0.57 | 0.39 | 0.36 |
| list-ops(1M)     | 113 | 70 | 3.16 | 2.85 |
| simd-sum(5M)     | 0.77 | 1.01 | 0.61 | 0.80 |
| simd-map(5M)     | 1.06 | 1.00 | 2.09 | 2.14 |
| simd-map2(5M)    | 1.02 | 4.85 | 1.82 | 8.86 |

binary-trees closed a third of its gap to both Rust and OCaml, on a slower CPU in absolute terms
too (526.3 to 414.9 ms). The simd-map2 ratio is the bimodality above, not a steady regression: its
min (20.7 ms) is level with Rust. Everything else is within the noise of a VM run. list-ops'
ratio to Rust is dominated by Rust's 1 to 2 ms fused loop and moves a lot on small absolute changes.

---

## Previous run: 2026-07-24, Apple M3 Max

The per-benchmark sections below, including their narrative and fix histories, are from this run
...
On the 2026-09-25 x86 run the SIMD picture is narrower than on the M3: March
leads or ties Rust on simd-sum and simd-map but trails OCaml and NumPy by about
2x on simd-map, and simd-map2 is currently bimodal (see "Latest run" above).
...
# A box with no opam switch: build the March rows with a given compiler
# (a release or nightly binary). The banner then says it is NOT this tree.
MARCH=/path/to/march bash bench/run_benchmarks.sh

# Record a run for committing: machine-readable rows plus a chart.
MACHINE_OUT=bench/results/<date>-<host>.jsonl bash bench/run_benchmarks.sh
python3 bench/chart.py bench/results/<date>-<host>.jsonl bench/results/<date>-<host>.svg
```

Unless `MARCH=` is set, the script pins `dune exec --root .` so it always measures the compiler in the
...
- `bench/ocaml/` — OCaml `.ml` sources (compiled with `ocamlopt`; the `simd_*`
  ones link `unix` for `Unix.gettimeofday`, via `ocamlfind` when present and
  otherwise via `ocamlopt -I +unix unix.cmxa`)
