...
- **A program that repeatedly passes a capture-free lambda (an anonymous
  ...
  function value already is. Measured on a 4,000,000-iteration loop calling
  `apply_it(fn x -> x * 2, i)`: allocations for the materialization step
  went from 4,000,000 to 0 and peak memory from about 131.5 MB to about
  3.0 MB, with identical program output before and after. Lambdas that
  capture a variable from the enclosing scope are unaffected by this fix
  and still leak — same open issue as the named-function capturing case
  below, tracked in `specs/todos.md`.
