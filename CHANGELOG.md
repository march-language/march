...
- **`record_get(r, k) == Some(<float>)` is no longer always false when
  ...
  *pointers*. Two equal floats in distinct boxes therefore always compared
  ...
  `expected_kind`, so it *selects* the encoding the runtime returns (boxed for a
  concrete `Float`, erased niche for an unresolved payload). An un-annotated
  ...
  boxed literal. The trade is the one `Vault(v)` handles already make: a single
  `record_get` application can no longer be used at two different payload types
  — meaningless at run time anyway, since the runtime commits to one encoding —
  and reading a field at several types still works by calling `record_get` once
  per type.
