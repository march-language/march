...
**Date:** 2026-06-21
**Status:** Draft — design approved in brainstorm; **P1 (transport + net-kernel L1/L2), L3 membership-CRDT, the L3 SWIM probe loop, the L5 global registry CRDT, and the L4 remote-call safety core implemented** (see `specs/progress.md` and `specs/todos.md` Done). Built as stdlib modules: `net_frame`, `node_identity`, `cluster_auth`, `handshake`, `net_kernel`, `peer_registry`, `cluster_conn`, `membership`, `swim`, `global_registry`, `global_pid`, `remote_call`; plus exposing `tcp_listen`/`tcp_accept` to the typechecker/eval. Pure logic eval-tested; socket I/O typecheck-clean + runtime-exercised. Remaining for L4: the enroll/stub/registry runtime mechanism (function-by-identity dispatch + marshalling stubs + `remote_ref` builtin — Compiler Touchpoint 1) and `Node.call` over the net-kernel. Also remaining: the SWIM runtime driver (random selection, real timers, frame send), ConsistentHash-partitioned registry ownership + Merkle anti-entropy, load-aware dispatch (L6), loopback integration test.
**Depends on:**
...
- **Cross-node monitors.** Extend local `monitor`/`Down` delivery over the net-kernel: remote process death → `Down(reason)`; failure-detector-declared node death → synthesized `Down(noconnection)` (standard OTP semantics). (Links were never implemented for either backend and were removed as permanently unreachable — see `specs/progress/2026-08-16-links-removed-monitors-are-the-fault-model.md` and `specs/progress/2026-08-17-compiled-link-builtin-still-unreachable.md` — so March's fault model, local or distributed, is monitors + supervisors only.)
