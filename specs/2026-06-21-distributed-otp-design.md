...
**Date:** 2026-06-21
**Status:** Draft — design approved in brainstorm; **P1 (transport + net-kernel L1/L2), L3 membership-CRDT, the L3 SWIM probe loop, and the L5 global registry CRDT implemented** (see `specs/progress.md` and `specs/todos.md` Done). Built as stdlib modules: `net_frame`, `node_identity`, `cluster_auth`, `handshake`, `net_kernel`, `peer_registry`, `cluster_conn`, `membership`, `swim`, `global_registry`; plus exposing `tcp_listen`/`tcp_accept` to the typechecker/eval. Pure logic eval-tested; socket I/O typecheck-clean + runtime-exercised. Remaining: the SWIM runtime driver (random selection, real timers, frame send), ConsistentHash-partitioned registry ownership + Merkle anti-entropy, load-aware dispatch (L6), loopback integration test.
**Depends on:**
