...
| parameterless **and pure** — pass a strict default unchanged | **38** |
| parameterless **and perform IO** — must migrate | **283** |

**Correction, and it matters because an earlier draft of this section argued
the opposite.** A first measurement reported 212 pure / 109 migrating and
concluded "this is not the ecosystem event the ladder doc implies." That was
wrong. It parsed `MARCH_DUMP_CAP_ROWS` output and treated "no row line
matched" as "pure", so every miss was silently counted as a pass. Measured by
the compiler's own stage-D error — the thing actually being decided — it is
**283 of 330 (86%)**.

The number should have been disbelieved on sight: it claimed two-thirds of a
corpus of examples and tests perform no IO, in a language where nearly every
example prints. The lesson is the one this repo keeps relearning — measure the
property you are deciding on, not a proxy that fails open.

So this IS a large migration, and the honest framing is: mechanical, exact
(the compiler computes each program's grant), and large.
...
```
143  IO.Console
 21  IO.Console, IO.Foreign
 14  IO.Console, IO.Spawn
  5  IO.Clock, IO.Console
  2  IO.Console, IO.Signal
  2  IO.Console, IO.NetListen
  2  IO.Console, IO.NetConnect, IO.NetListen, IO.Spawn
  2  IO.Console, IO.NetConnect
  2  IO.Console, IO.FileRead, IO.FileWrite
  1  IO.NetListen
  … (tail: one-off combinations)
```

Two things this distribution decides. **The overwhelming case is
`IO.Console` alone** (143 of 201 in these five corpora) — a single-parameter
migration. And **21 programs reach `IO.Foreign`**, which under stage B's rule
could only be granted `Cap(IO)`; D2's change is what lets them state
`Cap(IO.Console) + Cap(IO.Foreign)` instead of widening to the root.
...
**58 of the 201 measured programs (29%) need ≥2 capabilities.** Flipping the
...
Measured, and consistent with the argument rather than the basis for it:
`examples/csv_server.march` is the live instance of a `main` whose row carries
a transitive `unknown`, and it does not need to break. (An earlier draft cited
this as "1 of 33" from the same dump-parsing method that produced the wrong
migration count above; treat the witness as the claim, not the ratio.)
...
283 in-repo files, each a one-line signature change, generated by the autofix
...
- The 283-file migration verified by `specs/lang/types/check_types.sh` and a
