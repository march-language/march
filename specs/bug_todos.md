...
- [x] **#40** `lib/tir/llvm_emit.ml` (`coerce`, `ETuple`/`EAlloc`/`EStackAlloc`/`EReuse`) + `runtime/march_extras.c` (record builtins) — **Tag/untag mismatch on type-erased tuple/ADT slots** (two pre-existing native-codegen bugs blocking depot's `forge test`; cherry-picked from 20e53f2). Compiled code read tuple/erased-cell slots as `ptr` and untagged scalar views with an *unconditional* `ashr`, but `ETuple` and the `EAlloc`/`EStackAlloc`/`EReuse` fallbacks stored scalars in their NATURAL repr — so `match [("a",5)] do Cons((k,v),_)->v` returned `2` (5 ashr'd) and `true` matched as `false` in *every* compiled build, and `let (a,b)=(10,20); a+b` returned `15` instead of `30`. The depot symptom was an apparent hang in `test_depot_schema` "blank uses explicit defaults": a heterogeneous `opts` alist typed `List((String, Bool))` carried a `String` default ("user") in the `Bool`-typed slot; the `Bool`-specialized `alist_get_opt` `ashr`'d the raw string pointer (`0x14AF04430 >> 1 = 0xa5782218`), and `march_string_eq` then memcmp'd a garbage length.
  - **Fixed:** all tuple/erased slots now use the UNIFORM convention — scalars are low-bit tagged via `coerce i64→ptr` on store, matching the destructure fallbacks that already read+untagged them. `coerce ptr→i64` now untags **conditionally** (`ashr` iff the low bit is set): producers always tag scalars (always odd), so an even value in a scalar-typed view is a heap pointer flowing through a lying static type (dynamically-typed record/alist code) and is preserved verbatim — making tag→untag a lossless roundtrip for ALL bit patterns. `march_extras.c` record builtins aligned: `record_entries` pair-snd is uniform (was natural), `march_record_field_dyn` tags `'i'`, `record_put`/`record_from_list` normalize and downgrade kind `i→g` when a scalar-typed value's bits look heap-plausible.
  - **Reproduced** with `let (a,b)=(10,20); a+b` (pre-fix native `15`, post-fix `30`) and the depot `test_depot_schema` hang (now runs to completion).

- [x] **#41** `runtime/march_runtime.{c,h}`, `runtime/march_extras.c`, `runtime/march_http.c`, `runtime/march_message.c`, `lib/jit/jit_stubs.c`, `lib/jit/repl_jit.ml` — **Generic `to_string` on type-erased strings printed `#<tag:len>`** (second pre-existing native bug). `march_string`'s layout was `{rc; len; data}` with no tag word, so `to_string(x)` where `x : TVar` actually held a string emitted `march_value_to_string`, which read the `len` word at offset 8 (where ADT cells carry their tag) and printed `#<tag:N>` (N = string length) instead of the string. Depot's `Depot.Schema.parse_field_spec`/`define` do `to_string(typ)`/`to_string(pk)` on type-erased strings, so field types and the primary key were corrupted to `#<tag:6>`, breaking `field_type`, `primary_key`, and (via wrong `put_typed_default` dispatch) `blank()` defaults.
  - **Fixed:** `march_string` now shares the 16-byte `march_hdr` prefix — layout `{rc; tag; pad; len; data}` (len 8→16, data 16→24) — with every string carrying the reserved `MARCH_STRING_TAG` (`-1`, matching the long-standing cross-heap-copy convention already asserted in `march_message.c`). All string allocations route through a new `march_string_alloc` header-initializing helper so no site can leave the tag unset; `march_value_to_string` recognizes `tag == MARCH_STRING_TAG` and returns the string verbatim. The JIT/REPL string reader (`jit_stubs.c march_read_march_string`) and tuple pretty-printer (`repl_jit.ml`) were updated for the new offsets + the uniform tuple tagging from #40. Compiled code never bakes in the string layout (it builds strings via `march_string_lit`), so this is a runtime + JIT-marshaling change only. WASM keeps its own separate `march_string`.
  - **Reproduced** with depot `test_depot_schema`: `custom primary_key` + `field_type` tests (`#<tag:N>` → correct strings). Remaining depot failures are a separate generic-`to_string`-on-ADT-cells gap (`to_string(Some(0.0))` → `#<tag:1>`, needs ADT ctor-shape stamping) and a `record_entries` insertion-vs-sorted-order test expectation — neither is one of these two diagnosed bugs.

- [x] **#34** `runtime/march_runtime.c make_some_i64, march_string_to_int` — **Untagged Int payload in Some(Int)** (user-reported bug).  `make_some_i64` and the analogous path in `march_string_to_int` stored the `int64_t val` raw in the payload slot.  Under the uniform low-bit integer tagging scheme (commit 1696576), any polymorphic ptr-sized payload holding an Int must be pre-tagged with `(n << 1) | 1` so the compiler's `ashr #1` untag recovers the real value.  Without the pre-tag, extraction halved the value: byte offset 6 came back as 3, `"42"` parsed to 21.  Presence checks that only distinguished `Some(_)` from `None` kept passing, masking the bug (the user's observation that `String.contains` worked while `String.index_of` silently halved).
  - **Reproduced** with [examples/rc_option_int_tagged_payload.march](examples/rc_option_int_tagged_payload.march): pre-fix native prints `idx=3 / n=21`; post-fix prints `idx=6 / n=42` matching the interpreter.
  - **Fixed:** `make_some_i64` (used by `march_string_index_of` / `march_string_last_index_of`) and `march_string_to_int` now pre-tag the payload as `((int64_t)val << 1) | 1`.  Audited the other 24-byte alloc sites (`mk_ok`, `make_some_ptr`, file/csv helpers, actor_call) — all pass pointer-typed values (or unit `0`) that do not need tagging, so no additional fixes required.

- [x] **#38** `lib/tir/llvm_emit.ml` (lowering) + `runtime/march_runtime.c` (helpers) + `lib/tir/purity.ml` — **Native integer division by zero did not trap** (found while fixing the `__try_call` property-runner bug).  `int_div` / `int_mod` / `int_mod_euclid` lowered to raw LLVM `sdiv` / `srem` / `urem`, which is undefined for a zero divisor (SIGFPE on x86, garbage on some ARM paths); the interpreter raises `"<op>: division by zero"`.  Compiled code silently returned a junk value and kept running — so `int_div(100, 0)` produced `1` and a crashing property was reported as passing.  Two further layers masked the divergence even after adding a runtime trap: the constant case is computed inline, and — more importantly — `int_div`/`int_mod` were treated as **pure**, so DCE deleted `let _ = int_div(100, n)` (exactly the property-runner shape `fn n -> let _ = int_div(100, n); true`), eliminating the trap.
  - **Reproduced** with [examples/rc_checked_int_div_zero.march](examples/rc_checked_int_div_zero.march): pre-fix native prints `no-trap / no-trap / 14 / 2`; post-fix `int_div: division by zero / int_mod: division by zero / 14 / 2` matching the interpreter.  Also fixes `test/stdlib/test_check.march`'s "property that crashes is reported as a failure" (now passes natively).
  - **Fixed:** added `march_checked_idiv` / `march_checked_imod` / `march_checked_umod` runtime helpers that `march_panic` on a zero divisor with the interpreter's exact message (in test mode `march_panic` longjmps, so `__try_call` / the property runner catches it).  Both lowering sites (`EApp` and `ECallPtr`) now route through these helpers instead of emitting raw `sdiv`/`srem`/`urem`.  Added the four division/remainder builtins to `impure_builtins` in `purity.ml` so DCE/fusion preserve the now-observable divergence.  Non-zero behaviour is unchanged (signed for div/mod, unsigned for `int_mod_euclid`, matching the prior `urem` lowering).  Note: a pre-existing, separate divergence remains in `int_mod_euclid`'s `urem` lowering vs the interpreter's euclidean semantics for negative operands — out of scope here.

- [x] **#39** `lib/tir/llvm_emit.ml` (`is_int_arith` lowering) + `runtime/march_runtime.c` (helpers) + `lib/tir/purity.ml` — **Native `/` and `%` infix operators on Int did not trap on a zero divisor** (follow-up to #38, which only covered the `int_div`/`int_mod`/`int_mod_euclid` *builtin* call forms).  The operator forms `a / b` and `a % b` are lowered by the `is_int_arith` case, which still emitted raw `sdiv`/`srem` via `int_arith_op`, and `/`/`%` were still treated as **pure** — so `100 / 0` returned garbage natively while the interpreter raises `"division by zero"`, and `let _ = a / 0` was deleted by DCE before it could trap.  The interpreter uses the **bare** operator messages `"division by zero"` (for `/`) and `"modulo by zero"` (for `%`) — no `"int_div:"` prefix — see the `base_env` entries for `"/"` and `"%"` in `eval.ml`.
  - **Reproduced** with [examples/rc_checked_int_div_zero_ops.march](examples/rc_checked_int_div_zero_ops.march): pre-fix native prints `no-trap / no-trap / 14 / 2`; post-fix `division by zero / modulo by zero / 14 / 2` matching the interpreter.
  - **Fixed:** added `march_checked_div_op` / `march_checked_mod_op` runtime helpers that `march_panic` with the interpreter's bare messages; the integer branch of the `is_int_arith` lowering routes `/` and `%` through them (other ops stay native `add`/`sub`/`mul`).  Added `"/"` and `"%"` to `impure_builtins` in `purity.ml` so DCE/fusion keep the now-observable divergence — `"/"` also covers float division, which already traps on `0.0` via `march_checked_fdiv`, so marking it impure is correct.
