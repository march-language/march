...
| `sys_uptime_ms` | `() -> Int` | `IO.Clock` | Agree. Gated like `unix_time_ms` (it reads a clock; `witness.ml` already treats it as one); added to all three cap tables. `System` already declares `needs IO.Clock`. |
