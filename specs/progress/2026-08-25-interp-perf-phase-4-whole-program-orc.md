...
| `--debug`, `--coverage`, `MARCH_REPL_INTERP` explicitly select the interpreter | **PARTIALLY MET** — `MARCH_REPL_INTERP` and `--jit`'s own actor-detection fallback already route to the interpreter; whether `--debug`/`--coverage` currently force interpreted execution end-to-end (as opposed to just being compatible with it) was not re-verified in this task and needs its own audit before being counted as met |
