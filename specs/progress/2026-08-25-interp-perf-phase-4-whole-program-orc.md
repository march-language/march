...
| `--debug`, `--coverage`, `MARCH_REPL_INTERP` explicitly select the interpreter | **PARTIALLY MET** — final-review audit (2026-08-25): `--coverage` is a `march test` subcommand flag that never reaches the `--jit`/compile/interpret dispatch in `bin/main.ml`, and `MARCH_REPL_INTERP` is REPL-only — both are structurally safe (there is no code path where `--jit` could observe or override them). `--debug`/`--debug-tui`, however, is **not** routed back: `--jit --debug` silently drops the time-travel debugger instead of falling back to the interpreter or erroring. Verified: `--debug` alone prints `[debug] Trace recording enabled (buffer: N frames)`; `--jit --debug` on the same program prints nothing and JITs normally. This is the one open item for this criterion — suggested fix is to fold `!debug_mode \|\| !debug_tui_mode` into the `jit_run` guard (`bin/main.ml` ~4625) with a fallback notice, the same pattern already used for the actor-detection and shadowed-stdlib fallback arms. Not implemented here — audit/doc only. |
