...
- ✅ **Green thread scheduler (Phases 1-4)** — Phase 1-3: ucontext_t stackful coroutines, mmap'd stacks, reduction preemption, per-process FIFO mailboxes, M:N multi-thread with Chase-Lev work-stealing deques. **Phase 4**: Thread-local `march_tls_reductions` counter + `march_yield_from_compiled()` in `runtime/march_scheduler.{h,c}`; `lib/tir/llvm_emit.ml` emits inline load/decrement/store/icmp/br reduction check at every non-leaf function prologue (non-TCO) or TCO loop header; `is_leaf_callee` skips checks for builtin-only and `march_`-prefixed C-runtime calls; budget = 4000 reductions/quantum. 4 new `phase4_reduction_codegen` IR tests.
