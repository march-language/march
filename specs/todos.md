...
- ✅ **Green thread scheduler (Phases 1-3)** — `runtime/march_scheduler.{h,c}`: Phase 1: ucontext_t stackful coroutines, mmap'd stacks, reduction preemption. Phase 2: per-process FIFO mailboxes (send/recv/try_recv/wake), PROC_WAITING with spinlock-guarded recv protocol, process registry. Phase 3: M:N multi-thread with per-thread schedulers, Chase-Lev work-stealing deques, thread-local scheduler pointers, atomic status fields. Actor convergence: actors run on green threads (recv→dispatch loop), old worker pool removed. 11 C tests.
