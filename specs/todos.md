...
- ✅ **stdlib: DataFrame module** — `stdlib/dataframe.march`: columnar DataFrame with typed columns (`IntCol`, `FloatCol`, `StrCol`, `BoolCol`, `NullableCol`), row/column access, filtering via `ColExpr` DSL, `LazyFrame`/`Plan` evaluation engine, `GroupBy`/aggregation (`Count`, `Sum`, `Mean`, `Min`, `Max`), `inner_join`, CSV/JSON I/O (`from_csv_string`, `to_csv_string`, `from_json`, `to_json`), `col_describe`, `value_counts`. HAMT-based group-by hash table. 46 tests in `test/stdlib/test_dataframe.march` all passing.
