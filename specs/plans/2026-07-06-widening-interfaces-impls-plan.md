...
- **Corpus location:** reuse `specs/lang/types/{accept,reject}/` (continue the `tNN_` numbering — the interface/impl programs are typing conformance). Check the current highest `tNN` and continue from there.
...
**Deliverable:**
- Document how the `DImpl` check discharges **superclass/`requires` constraints** and any **`when`-clause** on the impl (re-grep the discharge steps in the DImpl arm), cross-referencing §2.1a's `discharge_constraints`. State what a superclass bound requires (e.g. `impl Ord(T)` requiring an `Eq(T)` impl — verify whether March enforces this live; if it does NOT enforce superclass presence, that's a finding — document + file).
- Document the **impl-head generality**: concrete `impl Iface(Dog)` vs generic `impl Iface(a)` — how the head type is instantiated/registered, and (cross-ref Task 4) that no specificity resolution exists.
- **Corpus:** ≥1 `accept/` (an impl with a satisfied superclass/`when` bound) and ≥1 `reject/` (an unsatisfied bound, IF March enforces it — else document the non-enforcement as a finding and skip the reject). Capture live.
...
**Deliverable:**
- Document **`derive`** (re-grep `derive`/`expand_derive` in `desugar.ml`): the closed set of five derivable interfaces (`Eq, Show, Hash, Ord, Json`), hardcoded string-match, unknown-target rejected; `Json` special-cased via `JsonTo`/`JsonFrom` pseudo-interfaces that skip normal validation. **FILE the gap:** `derive X for UnknownType` silently no-ops (exit 0, no diagnostic) — re-verify live, file in `specs/todos.md`.
