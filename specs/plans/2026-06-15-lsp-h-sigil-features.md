...
- ✅ **Tier 4** — `${…}` interpolation intelligence: hover/def/completion/diagnostics all work via existing position queries. Interpolation expressions are first-class AST nodes (parsed by `interp_parts` in `parser.mly`, each `${expr}` is a real `expr` with accurate source spans). No new position-to-interpolation index was needed — the existing `type_map`/`def_map`/`use_map` populated by the typechecker already cover all interpolation expressions. 8 new tests cover: (a) attr-value interp, (b) field-access interp, (c) let-bound local, (d) multiple interps on one line, (e) triple-quoted sigil, (f) type-error diagnostic positioned on the interpolation line, (g) island `props=${e}` hover/def. No `analysis.ml` changes were needed — the existing query path worked correctly for all cases.
