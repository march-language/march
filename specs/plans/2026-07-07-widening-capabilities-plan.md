...
- Add regression tests in `test/test_caps.ml` (model on existing cases) asserting the now-rejected programs reject and the pure ones still accept — RED pre-fix, GREEN after.
- **Corpus:** add `reject/t45–t47` — `cap pure` + `file_write` (t45); `cap pure` + `random_bytes` (t46); `cap deterministic` + `unix_time_ms` (t47), each with the pinned new message. (These are the F2 witnesses that were impossible pre-fix.) INDEX + counts.
...
**The bug (survey F3):** a `cap no_panic` module with a non-exhaustive `match` (which lowers to a runtime "no matching clause" panic) typechecks clean and then panics at runtime (P9c). `no_panic` covers named partial functions but NOT the match-non-exhaustiveness panic surface. The exhaustiveness checker ALREADY runs (it emits a non-exhaustiveness WARNING) — `no_panic` just doesn't consume its verdict.

**Deliverable:**
- Find where match exhaustiveness is computed (the pass that emits the "add a catch-all `_ ->`" warning). In `check_no_panic_module`, treat a non-exhaustive `EMatch` in a `cap no_panic` module as a panic site → ERROR (with a clear message, e.g. "a non-exhaustive `match` can panic at runtime; a `cap no_panic` module must handle every case or add a `_ ->` arm"). Reuse the existing exhaustiveness verdict — do NOT re-implement exhaustiveness.
...
**Verify:** the non-exhaustive-match program rejects after / accepted before; an exhaustive `cap no_panic` module still accepts; `scripts/run-tests.sh` FULL six-runner GREEN (watch for any stdlib `cap no_panic` module that now newly-rejects — investigate as in T5); `check_types.sh` all-pass; `check-docs.sh` 0.
