...
- **⚠️ F8 — participant HINT on stdout.** A `protocol` role that is not a declared `actor`/type emits a HINT (`typecheck.ml:~7064`) that interleaves with program stdout interp-side. This can break `verify.sh` (HINT present interp, absent compiled → mismatch). **Every golden/accept program must declare its protocol roles so NO HINT is emitted** (or the implementer must confirm the HINT appears byte-identically in BOTH backends). Determine the clean pattern live before writing corpus programs.
...
**Deliverable:**
- **Make the send and recv payload coercions symmetric.** Re-grep the send lowering (`lib/tir/lower.ml:352–442`, the `Chan.send`/`MPST.send`/`Chan.choose` arms) and the `chan_send`/`chan_recv` LLVM builtin emit path (`llvm_emit`/`llvm_builtins.ml:646–664`). Apply the SAME erased-i64 coercion at the send site that recv restores — i.e. coerce the payload `i64→erased` (the conditional TAG, inverse of recv's conditional untag) so a value stored by `send` round-trips through `recv` unchanged. (Equivalently: if you instead remove the conditional untag on the recv payload, the queue stores verbatim runtime values — pick whichever is consistent with the rest of the erased-i64/uniform-value convention; prefer matching how `record_put`/other builtin value args are coerced. Do NOT bare `inttoptr` — that corrupts per the erased-i64 note.) `chan_choose` sends an atom label — confirm atoms still round-trip after the change. `mpst_send` should get the same coercion (even though MPST run is OUT — keep the code path consistent).
...
5. **F8 HINT discipline:** every corpus program declares its roles so no HINT pollutes stdout (or the HINT is byte-identical both backends).
