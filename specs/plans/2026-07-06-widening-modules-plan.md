...
**Deliverable:**
- **The fix:** add the `entry.ex_public` gate to the `ExFn`/`ExValue`/`ExType`/`ExRecord` arms of `load_module_into_env`, mirroring the `ExCtor` arm's existing check + its message shape (`` … is private to module `X`. ``). Keep the message style consistent with the `ExCtor` one.
...
**Commit:** `fix(typecheck): enforce module visibility for ExFn/ExValue/ExType/ExRecord (private cross-module access rejected)`.
...
- **Visibility as a typecheck concept:** module typecheck exports only `pub_set`-filtered names as `"Mod.name"`; `load_module_into_env` now gates cross-module imports on `ex_public` (Task 1's fix). Document the rule: public names are importable/callable cross-module; private (`pfn`/`ptype`) are NOT (cite the now-correct rejection). This is the corrected behavior — describe it as the rule, cross-ref Task 1.
...
- **Corpus:** ≥1 `accept/` (a public cross-module type/value used with a qualified annotation); the private-access rejects are already in Task 1's corpus (cross-ref, don't duplicate). Optionally a `reject/` witnessing something new if a clean one exists.
