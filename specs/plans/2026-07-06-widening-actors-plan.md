...
- **DOCS slice — no compiler changes.** Only `specs/`. The two findings (affinity-not-enforced, call-timeout-unenforced) are documented + FILED, not fixed. A gap needing a compiler change is a FILED finding.
...
**Files:** `specs/lang/core-march.md` (supervision section); golden corpus; `specs/todos.md` (file the `Actor.call` timeout finding).
...
- **FILE the `Actor.call` timeout finding:** `Actor.call`'s timeout is UNENFORCED (`runtime/march_runtime.c:1762` — re-grep). File in `specs/todos.md` as an open `- [ ]` with the cite; note fixing it is runtime work, deferred.
- **Golden corpus:** ≥1 DETERMINISTIC supervision witness — a supervised child that crashes + restarts under `one_for_one`, producing a FIXED output (design the program so the restart + result are deterministic under `run_until_idle`, not timing-dependent). verify.sh MATCH. If a deterministic restart witness proves infeasible, document the restart semantics in prose + note why it's not golden-testable (like the OUT-of-scope races) rather than forcing a flaky program.

**Verify:** `verify.sh` all MATCH; `check_types.sh`/`check-docs.sh` 0; the supervision + epoch behaviors verified live; the call-timeout finding filed OPEN; supervision citations live.

**Commit:** `docs(spec): widen references — supervision (one_for_one + epoch restart) + file Actor.call timeout gap`.
...
- **Bookkeeping:** `specs/progress.md` — the slice-3 milestone (actors/messaging/supervision in both references, the deterministic-and-interp==compiled property, N golden + M types programs, findings). `specs/todos.md` — Done entry for the slice; confirm the affinity + call-timeout findings stay OPEN `- [ ]`; the message-name namespace is a design point (not a todo); note the next widening slice (session-types/protocols, or effects/capabilities) as queued.
...
3. **Findings filed, not fixed:** actor-affinity non-guarantee + `Actor.call` timeout are documented + FILED open; the message-name namespace is a documented design point. No compiler changes.
