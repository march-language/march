...
**Disposition:** extended the pre-existing standalone `test/test_oracle.ml` (`@oracle` alias, `test/dune:254-263`, already a separate slow-lane exe not in `@runtest`) rather than creating a new `test_conformance.ml` — it already did exactly Step 1's "enumerate bench/+examples, both-ways diff" job; the only real gap was the classification/triage contract (Steps 2-3). Corpus scope justified in the file's own header comment (see below) rather than extending to `test/`+doctests: `test/`'s 189+ files are compiler-internal fixtures, not conformance programs (see Step 1 disposition).

- [x] **Step 1 (scoped):** Corpus stays `bench/`+`examples/` top-level (61 programs, both-ways diff against `--compile` default flags) — the pre-existing enumeration. Deliberately NOT extended to `test/` (189+ compiler-internal fixtures: single-feature snippets, deliberately-erroring negative cases, fragments never meant to run standalone — would drown triage signal in noise) or stdlib doctests (need a dedicated `march>`-transcript extractor, not a `.march`-file sweep). Follow-up filed in `specs/todos.md`; justification stated in `test/test_oracle.ml`'s file-header comment per the plan's "OR ... justify" clause.
- [x] **Step 2: `nondeterministic_allowlist`.** Split the single `skip_set` into two named, commented lists: `nondeterministic_allowlist` (the original skip_set contents + `supervision_strategies` + `http_test`, one-line reason per group) and `known_divergence` (below). A program not in the corpus is simply not enumerated (directory walk); every enumerated program lands in exactly one of MATCH / KNOWN_DIVERGENCE / UN-TRIAGED FAILURE / SKIPPED / INTERP_TIMEOUT / INTERP_FAIL / COMPILE_FAIL(non-3) / RUN_FAIL(clean) — nothing un-triaged is silently dropped (loud-skip doctrine); an un-triaged divergence fails the sweep (exit 1).
- [x] **Step 3: Ran the sweep; found and filed real bugs.** Closed the core classification gap first (`is_failure` previously true ONLY for `Mismatch` — a signal-crash `RunFail` or ICE `CompileFail(3)` passed silently; new `is_divergence` catches both, `is_failure` = un-triaged divergence). Full sweep found 9 divergences, all triaged into `known_divergence` with cited `specs/todos.md`/`specs/progress.md` entries: `hello`/`list_lib` (to_string-on-container, pre-existing P1), 5 sorts (pre-existing "heapsort RC underflow" P0), `stats_basic` (pre-existing mono-limit P0, newly-confirmed second trigger), and **`dataframe_bench` — a NEW P0**, root-caused via `lldb` to an `EXC_BAD_ACCESS` in `march_incrc` reached through `Stats.mean`'s fold via `DataFrame.eval_agg`/`apply_group_by` (same RC-misclassification bug class as the sort family, distinct call site), filed in `specs/todos.md`. Isolated-HOME run: exit 0, 14 MATCH / 9 KNOWN_DIVERGENCE / 0 un-triaged failures / 0 stale-known-divergence `[WARN]`s.
- [x] **Step 4: Stays on the `@oracle` slow lane (unchanged), not wired into `@runtest`.** Confirmed the alias's dependency (`test_oracle.exe`) still builds clean. **Commit** `test(conformance): full-corpus interpreter-vs-compiled sweep with triaged exclusions (Phase 4)`.
