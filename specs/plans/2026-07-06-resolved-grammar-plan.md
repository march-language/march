...
- **§3 The `token_filter` pre-pass** — cite `lib/parser/token_filter.ml` (the `make` transformer, re-grep). This is the hard part: read the whole file and state, as normative rules, what it does to the token stream — the significant-**newline glomming** (when a newline separates block expressions vs is absorbed) and the **match/`cond`-arm-boundary lookahead** (how it distinguishes an arm boundary from a block continuation). Present each rule with a tiny before/after token-stream example. Be honest that this layer is not context-free and is why the EBNF (Tasks 2–5) takes a filtered token stream as input.
