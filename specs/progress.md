...
## Current State (as of 2026-07-10, Perihelion gameplay tuning — star variance, no-reversal orbits, arcing/capturable asteroids)

Six tuning requests on top of the two polish passes below. **Variance:** star radius range widened (14–22 → 12–28); orbit-ring size (`capture_radius`) now rolls independently of radius instead of a fixed 3.0× multiple; new `Star.speed_mult` (0.7–1.6×) scales that star's orbit angular speed. **No-reversal orbits:** `orbit_dir(i)` was alternating `+1.0/-1.0` by star index — every other capture flipped the player's spin, which read as a jarring reversal. Now a flat `1.0` for every star. **Spacing:** `gap_min/max` raised (120–200 → 160–260, stars were too close together); `max_dx()` (the vertical-weighting cap from the polish pass below) raised 160 → 220 to restore horizontal choice without giving up the vertical lean. **Arcing, capturable asteroids:** the biggest addition — asteroids now bend velocity toward the nearest in-range star each tick (a renormalized gravity-like bend, same shape as the player's own capture-assist), and on crossing a star's ring roll a 35% chance to get pulled into a permanent orbit (`Asteroid.mode : AsteroidDrifting | AsteroidOrbiting(Int, Float)`) instead of drifting through — a persistent hazard, same shot/collision rules as any asteroid, rendered in a distinct color. New `Combat.step_asteroids_go` threads `rng` through the capture roll, mirroring `step_ships_go`'s existing idle-timer-roll threading. Every `Star`/`Asteroid` literal across lib and all three test files updated for the new fields; full suite green; browser-verified over a sustained auto-tap session with zero console errors, including a direct pixel-color scan of the live canvas confirming multiple simultaneously-captured orbiting asteroids.

## Current State (as of 2026-07-10, js_emit self-recursive tail call fix — dropped return value on the JS target)

User-reported crash (`TypeError: can't access property "$", ... is undefined`) in `Perihelion.Combat.nearest_other_star`, the ship-migration code from the polish pass below. Root cause: Perceus keeps a self-recursive tail call in the shape `ESeq(call, EDecRC(dead_var)...)` specifically so `llvm_emit.ml`'s native TCO pass can recognize and rewrite it (see perceus.ml's "EXCEPTION: self-recursive tail calls" comment) — the JS backend has no such rewrite, so `js_emit.ml`'s generic `ESeq` handling emitted the call as a bare (unreturned) statement and the trailing `EDecRC` as a true no-op, silently discarding the call's return value. Trigger: a self-recursive tail call with a non-`Unit` return type that drops a heap-allocated binding unused in that branch (an `Int`/`Bool` head can't trigger it). Fixed with a new `is_rc_noop_only` check in `emit_stmts_impl`'s `ESeq` case: when the trailing side is pure RC bookkeeping (free on JS) and the leading side isn't, the leading side is the chain's real tail value and gets recursed into for proper `return` treatment. Regression: `test/native/js_self_tail_call_rc_drop.{march,expected}` (crashes pre-fix via a standalone repro, passes post-fix). Full suite green; the original trigger re-verified in-browser with zero console errors.

## Current State (as of 2026-07-10, Perihelion polish pass 2 — vertically-weighted ladder, no gameplay-star pop-in, decorative starfield)

Follow-up to the polish pass below, filed once the full-screen viewport exposed that star-ladder generation was tuned for the old 400px-wide world: on a 1600px+ viewport, `Level.next_star`'s x-placement (`lerp(margin, view_w - margin, tx)`, spanning the FULL width) produced shallow, near-horizontal hops instead of a climbable ladder. Fixed by bounding the x-jump to an offset from the previous star (`max_dx() = 160.0`, comparable to the 120-200 vertical gap) instead of the full viewport width, clamped via `Math.clamp_float`. Separately, `Core.top_up`'s "≥2 stars ahead" fixed-count trigger could leave a tall full-screen viewport staring at empty space above the ladder — a captured star at the previous headroom's edge would pop into existence mid-frame. Replaced with a distance-based trigger (`lookahead_screens() = 1.5`, i.e. keep the ladder generated 1.5 screen-heights above `camera_y`) called every frame from `step_playing`, not just on capture. Finally, a purely decorative background starfield (`Perihelion.draw_starfield`) was added to the shell: a deterministic sin/frac grid-cell hash (same trick as the asteroid polygon jitter) scatters small dim dots across the full viewport width, recomputed fresh from `(camera_y, view_w, view_h)` every frame rather than stored in `Game` — so it fills the wide empty margins around the narrow gameplay ladder and, by construction, can never pop in either. One test assertion relaxed from an exact star count (tied to the old fixed-count top-up policy) to a >-comparison, since generation count now depends on the live viewport. Full suite green; browser-verified at 1600×900: starfield edge-to-edge, visibly steeper ladder, no pop observed while climbing several stars.

## Current State (as of 2026-07-10, Perihelion polish pass — cursor-aim fire, full-screen viewport, polygon asteroids, orbital ship migration)

**Six requested polish items, five shipped** (one — keep-running-past-the-top-of-the-ladder — was retracted by the user before implementation). `Game` gained `view_w`/`view_h : Float` fields, threaded through `Core.update`/`fresh_run`/`new_game_saved` and `Level.next_star`/`initial_stars`, replacing the old module-constant `view_w()`/`view_h()` (renamed `default_view_w()`/`default_view_h()`, still used as the pre-boot default). Two new poll-based `Dom` externs following the `taps()` idiom: `pointer_pos(el)` (mousemove-buffered, defaults to element center) and `window_size()` (direct `window.innerWidth`/`innerHeight` read). `Combat.fire` now aims at the live cursor instead of firing radially outward, falling back to the old star→ball radial direction only when the cursor sits exactly on the ball (degenerate zero-length aim). The tether line between star and ball is gone (`draw_tether` deleted). Asteroids render as jittered polygons (deterministic sin/frac hash off a new `Asteroid.shape_seed : Float`, collision still circular by design). Enemy ships migrate between stars via a `ShipMode = ShipOrbiting(Float) | ShipFlying(Float, Float)` state machine mirroring the player's own orbit/release/fly/capture cycle, instead of sitting in a fixed orbit.

**Real bug found and fixed, not a preview-tool artifact.** `main()`'s boot sequence read `Dom.window_size()` synchronously at module-load time; in this environment `document.readyState` is still `"interactive"` at that point and `window.innerWidth`/`innerHeight` read as `(0, 0)`, silently baking a 0×0 viewport into the initial star ladder (`s0.x = view_w / 2 = 0`) while every subsequent per-frame HUD/background draw picked up the correct size (since `tick()` re-polls every frame) — producing a star pinned at the canvas origin underneath an otherwise-correctly-sized HUD. Root-caused with temporary `console.log` instrumentation in `march_dom.mjs` (removed before commit) proving the `(0, 0)` read at `readyState: "interactive"`. Fixed by deferring the entire boot body (window-size read, canvas resize, `new_game_saved`, first `tick()` call) into a `Dom.on_frame` callback — the same deferred-callback pattern the per-frame loop already used — guaranteeing a layout pass has completed before the first real read.

Verified: `test_core`/`test_combat`/`test_level` (177/154/141 tests) green; full suite green; browser-verified via CDP `eval`-dispatched `pointerdown`/`mousemove`/`keydown` events (`preview_click` sends a bare `click`, not the `pointerdown` the game listens for) — no tether, jagged polygon asteroids, cursor-aimed fire, live full-viewport resize with correct centering, matching the `playstarfling.com` reference composition. Design: `docs/superpowers/specs/2026-07-10-perihelion-polish-design.md` (local-only).
