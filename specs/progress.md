...
## Current State (as of 2026-03-25, post SIMD HTTP parser + Phase 5 per-process heaps + message passing + zero-copy response builder)
...
- **LSP code actions P1.8 / P2.10 / P3.4** — (P1.8) "Assign to `_`" QuickFix for `unused_binding` diagnostics: replaces the binding name with `_` (discards result, distinct from the existing "Prefix with `_x`" action). (P2.10) "Remove unused import" QuickFix: `warn_unused_imports` in `typecheck.ml` now emits `warning_with_code ~code:"unused_import"`; code action deletes the whole import line for whole-module diagnostics, or removes just the unused name from a `{a, b, c}` list for specific-name diagnostics (with comma-aware source scan); `unused_import` registered in `fix_registry`. (P3.4) "Wrap with inspect" / "Remove inspect" `RefactorRewrite` actions: wrap uses `type_map` to find the innermost expression at the cursor and inserts `inspect(expr, "label")`; remove scans source backwards for `inspect(` on the current line and replaces `inspect(inner, "label")` with `inner` by counting parens to find the matching close. +9 tests (107 total).
