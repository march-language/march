...
- **Cross-language benchmarks** — `bench/elixir/`, `bench/ocaml/`, `bench/rust/` contain idiomatic implementations of fib(40), binary-trees(15), tree-transform(depth=20×100), and list-ops(1M); `bench/run_benchmarks.sh` compiles all, runs 10 times, reports median/min/max; results in `bench/RESULTS.md`. **Post-TCE/fusion results (2026-03-23):** list-ops 67 ms (was 117 ms, -43% via stream fusion); fib/binary-trees/tree-transform fail to compile under TCE (tree-recursive — need CPS rewrite or escape hatch). Pre-TCE highlights: FBIP 7.5–19× on tree-transform; March ≈ Rust on fib.
