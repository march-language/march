...
**Three stacked bugs, and the compiled HTTP server served nothing.** A
compiled `HttpServer` panicked `non-exhaustive pattern match` on request 1.
Fix that and it returned `200 OK` with an empty body, no plug having run. Fix
*that* and it segfaulted (exit 139, silently) on request 2. Both server paths
were affected — the default thread-pool one and the opt-in event loop. Every
one of the three was compiled-only; the interpreter served the same program
correctly throughout, which is exactly why the `http_server` tests (all
interpreted, `adversarial-regressions 48`/`49`) never saw it.
...
*Bug 2 — boxed vs. raw `Bool`.* `make_bool` in `runtime/march_http.c`
allocated a 16-byte object and set a tag, per a comment claiming "March Bools
are heap objects with just a header". A `Bool` field of a *boxed* ADT is a raw
i64 0/1 (the `(v<<1)|1` tagging in `lib/tir/repr.ml` governs niche *payloads*,
not ordinary fields). The pointer was non-zero, so `if halted(conn)` in
`run_pipeline` was true for every fresh conn and the pipeline returned it
immediately. `march_ffi.c`'s `march_make_bool` had the encoding right all
along; only this local copy was wrong.
...
it were wired up it would have passed against bug 2 (a well-formed empty 200)
and against bug 3 (which needs a *second* request to show up).
