...
## Current State (as of 2026-03-28, post WASM Tier 4 validation + CorrelationId middleware + scaffolder fixes + Bastion adversarial test suite)
...
- **14 pre-existing failures** in `repl_jit_regression`/`repl_compiler_parity` (unrelated to Bastion work); all other suites pass. Full list: (app entry point + HAMT Map/Set/Array + tap bus + REPL/compiler parity + MPST + REPL JIT fix + LSP Phase 1 + LSP Phase 2 + tail-call enforcement + structural recursion refinement + stream fusion + type-level nat solver + built-in testing library + March-native stdlib tests + TCE structural recursion warning + Random/Stats/Plot stdlib + describe keyword + FFI interpreter dispatch + JIT bitwise builtins + doctest extraction + **TCO loop transformation in LLVM codegen** + **DataFrame Phase 7** + **constant propagation** + **Mutual TCO** + **borrow inference** + **known-call** + **struct update fusion** + **escape analysis** + **Phase 5: per-process heaps + message passing** + **Phase 4: reduction counting in compiled code** + **Phase 4: lazy stack growth** + **Vault sharded KV store** + **Bastion.Cache + Bastion.Depot middleware**):
  - `test_march.exe`: 1180 tests, 14 pre-existing repl_jit/repl_compiler_parity failures
...
- **Bastion adversarial test suite** — 346 adversarial tests across 7 files in `examples/bastion_tests/`: routing (42), caching (37), sessions (35), csrf (44), dev-dashboard (64), html (54), islands (70). Covers path params, ETags, Vault fragment cache, cookie/vault session backends, CSRF token injection/replay, BastionDev SSE live-reload, HTML escaping, WASM loading, island CSS scoping, cross-island messaging. Fixed `stdlib/islands.march` bug: `_island_runtime_script` was embedding the full JS blob instead of serving from `/_bastion/islands.js`. All 346 tests pass. `examples/bastion_tests/RESULTS.md` documents all findings.
- **CorrelationId middleware + uuid_v4 builtin** — `stdlib/correlation.march`: `CorrelationId.assign/1` middleware reads `X-Request-ID` header or generates a UUID v4, stores in `conn.assigns["request_id"]`, echoes in the `X-Request-ID` response header, and injects into the Logger context via `Logger.with_context` so all log calls in the request process include `request_id` automatically. `CorrelationId.request_id/1` accessor, `CorrelationId.clear/0` to reset Logger context. `uuid_v4` builtin added to eval: RFC 4122 v4 UUID (version nibble=4, variant=10xx). 15 tests in `test/stdlib/test_correlation.march`. Spec: `specs/bastion/logging.md`.
- **WASM Tier 4 browser target — pipeline validated** — `wasm32-unknown-unknown` codegen produces working WASM binaries. Validated end-to-end: `march_island_render`, `march_island_update`, `march_island_init`, `march_alloc_export`, `march_dealloc`, `march_island_render_html`, `march_island_string_length`, `march_island_string_data` all export correctly and pass JS `WebAssembly.instantiate` tests. Fixed 4 compiler bugs: (1) `find_fn` island export matching was catching `Vault.update` instead of user's `update` — narrowed to user module prefix; (2) vault builtins missing from `is_builtin_fn`/`mangle_extern`/preamble — causing broken `%vault_update.addr` IR; (3) `ECallPtr` to known builtins wasn't short-circuited to direct calls; (4) `build_wasm.sh` pointed to native `march_runtime.c` instead of `march_runtime_wasm.c`. Added `memset/memcpy/memmove` stubs and vault WASM stubs to `march_runtime_wasm.c`. Integrated into `Islands.loadWasmModule` stub in `march_islands.js` (both IOList-returning and String-returning render paths).
- **forge bastion scaffold fixes** — Refactored `scaffold_bastion.ml` from `forge/lib/` to `bastion/lib/` (eliminates circular deps, local `mkdir_p`). Fixed generated templates: router uses `Get/Post` type constructors (not `:get/:post` atoms); app source lambda uses `fn conn -> do...end` for multi-statement body; test template uses proper stubs instead of non-existent `HttpServer.test_conn`. Updated `cmd_bastion_routes` arm parser to recognise both `(Get, ...)` constructor style and `:get` atom style. All 20 `test_forge.exe` forge-bastion tests pass.
