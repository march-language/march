...
- **DataFrame stdlib module** — `stdlib/dataframe.march`: columnar DataFrame with typed columns (`IntCol`, `FloatCol`, `StrCol`, `BoolCol`, `NullableCol`), row/column access, filtering via `ColExpr` DSL, `LazyFrame`/`Plan` evaluation engine, `GroupBy`/aggregation (`Count`, `Sum`, `Mean`, `Min`, `Max`), `inner_join`, CSV/JSON I/O, `col_describe`, `value_counts`. HAMT-based group-by. 46 tests all passing.
