...
## Current State (as of 2026-07-13, uniform apply-fn ABI stages 1-4 landed)

**LANDED — the uniform apply-fn arg ABI (stages 1-4 of specs/plans/2026-07-13-uniform-apply-abi.md).** Every closure apply takes plain ptr-slot args (scalars tagged, floats raw bits, heap ptrs raw) and every consumer encodes to match: ECallPtr dispatch, known_call'd direct EApp, the (newly consolidated) clo_wrap trampolines with in-wrapper decode, the C runtime's raw closure callers (tagged dummies), and the JIT stdlib-prelude cache key (abi2 tag closes cross-time skew). The calling convention no longer depends on the inference outcome — the curried-comparator SIGBUS class is closed at the root for INTs (generalized int comparators sort correctly now). IR pin test flipped to assert the uniform shapes; bench checksums + list_ops timing unchanged (0.10s median x10). **Stage 5 (lift the monomorphism restriction) is BLOCKED and the MR STAYS:** the float gate caught a pre-existing repr-level bug — generalized float closures crash (Perceus RC on raw float bits in erased slots) or silently mis-sort (generic <= integer-compares float bits; pre-flip the annotated form mis-sorted SILENTLY). Filed with mechanism + repro; fix = float boxing at erasure boundaries or runtime type witnesses.
...
## Current State (as of 2026-07-13, Actor.call plane backend-identical — dispatch + timeout fixed both backends)

**FIXED — the whole `Actor.call` P1 cluster, both backends.** (1) The interpreter now dispatches calls exactly like the compiled runtime: positional tag-routing of the zero-arg sentinel to the handler at that ctor index, caller bound as the handler's single arg (eval.ml ctor_index_tbl; the legacy `on Call(ref, msg)` form is retired). (2) The compiled side's silent call-drop is root-caused and fixed: F19's globally-unique actor-msg tags (base 0x0100_0000) made march_actor_call's sentinel-tag stamp miss every dispatch switch — codegen now registers each actor's msg-tag base at the actor-record alloc (march_actor_set_call_base) and the runtime translates sentinel index → base+index. (3) `timeout_ms` is now ENFORCED compiled (deadline-bounded yield-poll via new march_sched_try_recv2; `<= 0` = wait forever). test/native/actor_counter prints value=5 on BOTH backends; new test/native/actor_call_timeout pins timeout on both; node_discovery's build-time SIGSEGV cured (downstream of dropped calls); full `dune build --root .` completes for the first time. Still open on the actor plane: compiled epoch-Cap send validation, Pid[state] spawn typing.
...
## Current State (as of 2026-07-13, DataFrame P0 FIXED — crash + pre-existing group_by wrong answer)

**FIXED — the last open P0: DataFrame `group_by`/`Stats.mean` compiled SIGSEGV, plus the separate wrong-answer bug hiding behind it.** (1) Crash: `lower.ml` span-prefers the typechecker's numbered-family types for polymorphic nested-fn param annotations (aligning the tvar family mono's local subst covers with the body's) and `perceus.ml`'s dead-branch-var dec resolves TVar-typed field vars' concrete ctor field types (`resolve_case_field_ty`, mirroring llvm_case's IncRC side) — no more RC ops on raw Float bits. (2) The "Groups: 0 regression" that forced the 2026-07-12 revert was a MISATTRIBUTION — a pre-existing silent wrong answer reachable only past the crash: bare ctor `Row` is owned by both boxed `DataFrame.Row` and niche-shaped `Csv.CsvRow`, and codegen's bare-name niche recovery decoded a Row box as CsvRow's identity through the erased `Cons(Row(pairs),_)` sub-var (payload = the box pointer, tag 0 read as Nil → empty). Fixed by qualifying ambiguous bare constructor branch tags to "Type.Ctor" from the typechecker's own pattern resolution (new `ambiguous_bare_ctors` + `resolved_pattern_ctor_types` tables; `lower_match.ml` consumes them; `qualified_br_key` resolves exactly like the encode key). `dataframe_bench` now byte-identical to the interpreter modulo timing lines (Groups: 5, join 800); suite green (813 + 2 new Slow adversarial regressions, stdlib runner now 815); goldens 50/50; snapshots 29/29 UNCHANGED. Still open: the sort-RC family (`Stats.median([floats])` is a fresh minimal repro; `heapsort` flaky-not-fixed) and to_string-on-container.

## Current State (as of 2026-07-13, FBIP un-starved + §4.16 E-Reuse/RC-Mode doc slices written)

**FIXED — FBIP reuse had been silently disabled program-wide; §4.16's two "blocked on compiler work" doc slices now written on top of the fix.** Investigating why golden `g50_fbip_reuse_semantics`'s TIR showed zero `reuse` ops revealed that since `join_points.ml` began lifting a match's panic default arm into a `$jp_clo` closure, every real arm carries a `dec_rc $jp_clo` between its let chain and its tail alloc — and `try_fbip_sink` (`lib/tir/perceus_fbip.ml`) only traversed ELet nodes, so the scrutinee-free dec never reached the alloc. Even the `fbip_dead_binding_reuse` TIR snapshot had the starved shape pinned in as if correct. Fix: one new sink case hopping RC ops on OTHER variables (sound: RC ops don't read fields; delaying a dec only delays a free; the aliasing corner is caught by EReuse's runtime RC==1 uniqueness branch). Also added a fail-loudly full-overwrite guard at EReuse emission (arg count must equal the resolved ctor's field count). `bench/tree_transform` 9.67s → 0.98s (~10×), byte-identical; full suite green (813 stdlib-runner tests); golden corpus now **50/50** (`g50` new — the interpreter performs no reuse, so interp==compiled byte-identity IS the semantic-equivalence witness); snapshot deliberately regenerated, diff reviewed. Docs: `core-march.md` §4.16 gains **E-Reuse** (FBIP reuse-preserves-semantics) and the **RC-Mode** atomic/local selection rules (static actor-sent partition = performance; the runtime's scheduler/flagged-thread promotion = correctness); `specs/atomic-rc-design.md` reconciled Draft → largely-implemented-in-modified-form (no `rc_mode` field or escape pass was ever built; the HTTP borrow workaround is gone, replaced by `march_rc_set_thread_concurrent`); new codegen pin `test_actor_sent_rc_ops_atomic_locals_stay_local` asserts both `march_incrc(`/`march_incrc_local(` dispatch spellings in emitted IR (codegen runner now 405 tests).
...
**FIXED — interface default method calling a sibling method compiled always returned the same answer.** Root cause was a plain arity mismatch: desugar's `inject_defaults` wrapped the default's lambda in a ZERO-param clause, lowering to a 0-arity TIR fn returning a closure, while dispatch call sites called the mangled impl at the METHOD's arity — args silently dropped, the closure POINTER bound as the Bool result (always truthy). Fix: eta-expand lambda defaults into regular n-param method clauses (`lib/desugar/desugar.ml`), making injected defaults shape-identical to hand-written methods. Regression test `test_compiled_default_method_sibling_call`.

**FIXED — "Monomorphization limit reached: List.fold_left > 512 specializations" ICE (nested-closure repro + examples/stats_basic.march).** The real bug was in the TYPECHECKER, not mono: the scope-blind, name-keyed `fn_arities` direct-call arity check matched a user top-level `fn f(m)` (arity 1) against STDLIB fold_left's local param `f : b -> a -> b` shadowing the name; the anti-shadowing guard (`count_arrows >= arity`) passed on the curried param type, producing a false arity error AT A STDLIB SPAN — silently filtered by bin/main.ml — whose recovery typed `f(acc, h)` as a partial application; the recursive call's occurs failure then linked the type var to TError, poisoning the type_map and feeding mono a `+1 Fn`-wrapped substitution per generation. Fix: `bind_var`/`bind_linear` remove the bound name from `fn_arities` (lexical shadowing, scoped by the functional env), with top-level registration sites re-adding their own entries (`lib/typecheck/typecheck.ml`). The silent-poisoning class was wider than the ICE: 5 distinct filtered occurs-failures fired compiling the repro, 1 even on a healthy program. `stats_basic` now compiles (oracle known_divergence entry moved to the still-open to_string-on-container reason); `test_compiled_recursive_closure_capture`'s documented skip is now a hard-failure regression guard and the test passes.

**STALE ENTRIES CLOSED (verified already fixed, never checked off):** compiled `Task.await` heap-payload OOM (fixed by the 2026-07-06 Result-path tagging widened guard; now pinned by new `test_compiled_task_await_heap_payload`) and compiled `tcp_recv_http` non-exhaustive-match panic (fixed 2026-07-08 by `ed778f10`'s make_ok/make_err wrapping; live-verified against a real HTTP server, success and error paths).

**INVESTIGATED, NOT FIXED — DataFrame group_by/Stats.mean compiled SIGSEGV.** Now has a 6-line DataFrame-free minimal repro (`Stats.mean(xs)` on a SHARED `List(Float)`) and a precise 3-layer root cause (pattern-matrix sub-vars typed `TVar "_"` → conservative `needs_rc` → RC ops on raw double bits; nested-fn params typed in the annotation tvar family vs the body's numbered family so mono's local subst misses body types; perceus's dead-field DecRC lacking the ctor-field-type resolution llvm_case's matching IncRC already does). A 3-part fix eliminated the crash end-to-end but introduced a silent wrong answer (GroupBy `Groups: 0` — a stale `ty_of_span` on nested-fn params breaking group-key hashing, the exact staleness `_fn_param_types` exists to avoid) and was REVERTED per the leak-not-crash/never-trade-bugs discipline. Full analysis + recommended fix order in specs/todos.md.

## Current State (as of 2026-07-12, finding B18 FIXED — TCO loop-carried-value release-timing bug; runtime uniqueness check in llvm_emit.ml)

**The "Toml multi-digit integer truncates to its first digit when compiled" bug turned out to be one narrow instance of a general, stdlib-independent TCO/RC bug, now fixed** — reproducible in ~10 lines with zero Toml involved: a self-tail-recursive function whose parameter gets rebound each iteration to a freshly-derived value (e.g. `let rest = String.slice_bytes(s, ...); drain(rest, n+1)`) used to run exactly one real iteration then read freed memory, compiled-only; now correct both interpreted and compiled.

**Root cause:** `lib/tir/llvm_emit.ml`'s TCO loop-back-edge codegen executes a Perceus-emitted trailing `dec_rc rest` *before* installing `rest` as the next iteration's parameter — but `rest` IS that next value. Under the borrowed-argument convention, that decrement is only sound once a real call/return boundary has passed (true recursion: the decrement fires only after the entire nested call has returned, by which point nothing needs the value anymore); TCO replaces the call with a jump, erasing that boundary, so the decrement can free the value one instruction before the loop reads it back.

**First attempt and why it was reverted:** unconditionally redirecting the decrement to the OLD (pre-overwrite) parameter value fixes the exact repro above cleanly, but is unsound in general — it introduced a real regression in the pre-existing `test_compiled_toml_section_4keys` test. `Toml.table_get`'s tail-recursive walk destructures its own `pairs` parameter (`Cons(p, rest) -> ...`), and that `rest` is a Perceus-inserted `inc_rc`-duplicated alias into a *shared, persistent* list — the same parsed table gets walked from the top again by every subsequent `get_str` call. Unconditionally redirecting the decrement there released a cons cell (`pairs` itself) a later, independent call still needed, instead of leaving the already-correct original decrement of `rest` (which only cancels its own extra `inc_rc`) alone.

**The actual fix:** the two shapes are statically indistinguishable at the TCO back-edge, but differ at RUNTIME by one fact — does the new value have any other live reference besides the one about to be released? `emit_tco_redirected_dec_chain` now loads the new value's own current refcount right before releasing it (mirroring the existing FBIP reuse-check pattern): RC == 1 (no other owner, e.g. `drain`'s fresh allocation) → redirect the release to the superseded old parameter-slot value; RC > 1 (a persistent structure holds another reference, e.g. `table_get`'s aliased field) → release the new value exactly as Perceus originally emitted. Both branches are correct and composable — no Perceus-level change was needed.

**Verification:** `drain=4` both backends; `test_compiled_toml_section_4keys` still passes; full suite (808 tests) green; `bench/tree_transform.march` timing unchanged (~8.4s, within noise) confirming negligible perf cost from the extra runtime check. New regression tests: `test_compiled_tco_borrowed_fresh_value_no_premature_free` and `test_compiled_yaml_get_str_no_hang` (`test/test_stdlib_suite.ml`).

**Also fixed, same root cause, no Yaml-specific changes needed:** `Yaml.get_str`'s divergence, originally filed as "returns `None` compiled vs `Some(...)` interpreted," was actually a compiled-only **infinite hang** (confirmed via bounded-timeout kill) — the same B18 fix resolves it (verified via direct `parse_exn`, the `~yaml"..."` sigil form, and a 4-key multi-lookup variant).

**Test-infra fix along the way:** `test_mutual_tco_borrowed_arg_decref_on_live_path` (`test/test_codegen.ml`, the B7 regression guard) needed its IR-block-scanning heuristic widened (the runtime check splits what used to be a single block into a small diamond), and a real `Str.match_end()` global-state clobbering bug was found and fixed in the process (a nested `Str.search_forward` inside the window-scan helper was corrupting the outer loop's match-end, causing an infinite loop before the fix).
