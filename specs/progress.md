...
- **`--dump-phases` compiler flag + phase viewer** — `march --dump-phases --compile file.march` serializes each IR stage to `march-phases/phases.json` as a JSON graph (`{ phase, nodes, edges }`). Nodes carry `id`, `label`, `type`, and `metadata` (calls list, RC op count, arity, etc.); edges carry `source`, `target`, `label`. Phases emitted: `parse` (AST, user module only), `tir-lower`, `tir-mono`, `tir-fusion`, `tir-defun`, `tir-known-call`, `tir-perceus`, `tir-escape`, `tir-opt`. `lib/dump/dump.ml`: JSON serialisers for `March_ast.Ast.module_` and `March_tir.Tir.tir_module`; call-edge extraction from both AST expr trees and TIR bodies; RC op counting for Perceus diff visibility. `tools/phase-viewer.html`: self-contained Cytoscape.js visualiser — phase selector sidebar, interactive node/edge graph, diff mode (highlights added/changed/removed nodes between any two phases), node info panel showing metadata and call list. `forge build --dump-phases` and `forge run --dump-phases` pass the flag through via `cmd_build.ml` / `cmd_run.ml` / forge bin. Usage: `march --dump-phases --compile bench/binary_trees.march && open tools/phase-viewer.html`.
