...
- **Module load-order independence (global module registry)** — `lib/eval/eval.ml`: `module_registry` hashtable populated as each `DMod` is evaluated; `EField` qualified lookups and dotted `EVar` fallback check the registry at call time so cross-module references work regardless of definition order in the source file. Registry cleared per `eval_module_env` run.
