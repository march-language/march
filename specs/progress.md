...
- Record patterns (`{ x, y: p }`) in `match` arms, `let` bindings, and function parameters — destructures a record's fields, with bare-name punning (`{ x }` for `{ x: x }`). Field lists must currently name every field of the scrutinee record exactly; partial field lists remain future work.
