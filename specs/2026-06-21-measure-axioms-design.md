...
3. ✅ **M-c — measures calling measures + mutual recursion (SHIPPED 2026-06-21).** The self-recursion-only restriction is lifted: a measure may call another *axiomatisable* measure, with the argument still required to be a structural component (every measure call — self or cross — decreases, so the whole system is structurally decreasing and total). Which measures are axiomatisable is resolved by a **greatest fixpoint** (start with every shape-OK measure; drop any whose arms fail to translate against the current candidate set, until stable), so a measure that calls a non-axiomatisable one cascades to symbolic. All `declare-fun`s are emitted before any axiom (forward/mutual references resolve). Mutually-recursive *datatypes* are handled by emitting the transitive ADT closure in a **single combined `declare-datatypes`**, and constructor fields of any registered ADT (not just self) are reflected to that ADT's sort so cross-ADT calls are well-sorted. Verified: `tsize`/`fsize` over mutually-recursive `RTree`/`RForest` compute structurally (`tsize(RNode(_,FCons(RNode(_,FEmpty),FEmpty))) = 2`). Still self-recursion in the gate (§4); a non-structural *mutual* cycle isn't a hard error but stays unaxiomatised (sound symbolic fallback). Deferred to later: a measure call on the whole matched value (`A(t) = B(t) + …`) and constructor-application arguments.
4. **M-d — relational postconditions** (`size(insert(t,x)) == size(t)+1`) where provable by E-matching; document where a user lemma is still required. (Note: postconditions currently reflect measures only *symbolically* — relational reasoning needs reflecting the *result* of the operation as a datatype term, the core M-d work.)
...
## 10. Performance (measured 2026-06-21)

Measured on a cold VC cache (`--check`), worktree dev compiler:

| Workload | Axioms on (default) | `--no-measure-axioms` |
|---|---|---|
| 40 distinct concrete-structure measure-bound call sites | **69.9 s** cold / 0.5 s warm | 0.45 s (catches 0 of 8) |
| 150 plain Int/Bool VCs + a measure defined | **1.08 s** | — |

Findings:
- **Quantified + datatype reasoning is expensive per VC** (~1.75 s/VC averaged over the stress file; many in-/out-of-bounds queries approach the 3 s solver timeout). The BLAKE3 VC cache makes warm rebuilds ~150× faster, so iterative dev is fine; a cold CI build of a measure-heavy module is not.
- **The cost is now isolated to measure-using VCs.** The axiom preamble is attached to a VC *only* when its predicate references an axiomatised measure (postconditions, which reflect measures symbolically, never get it). So a codebase with many refinement VCs but few measure call sites pays the axiom tax only on the few — 150 plain VCs alongside a defined measure cost ~1 s, not 150× a quantified query.
- **`--no-measure-axioms`** reflects measures symbolically (pre-M-a behaviour) and also disables the §4 gate (which exists only to keep the axioms sound). It changes only diagnostics, never the compiled artifact, so it is not part of the CAS cache key.

Decision: keep axioms **on by default** — non-measure code is unaffected, measure-bound call sites are rare in practice (the stdlib hardening pass found clean checkable consumers scarce), and the warm cache covers iterative dev. `--no-measure-axioms` is the escape hatch for measure-heavy cold builds. Possible future tuning: a lower per-query timeout, or sharing/committing the VC cache.
