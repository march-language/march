...
- `lib/repl/repl.ml` — add debug commands to TUI command dispatch
...
- [ ] **Step 4: Add DBG token to parser**
...
- [ ] **Step 3: Add EDbg passthrough to desugar.ml**
...
- [ ] **Step 4: Add EDbg to typecheck.ml**
...
- [ ] **Step 3: Add ring buffer and debug_ctx types to eval.ml**
...
(** [ring_abs_index r i] converts logical index i to absolute array index. *)
let ring_abs_index r i =
  ((r.rb_head - 1 - i) + r.rb_cap * 2) mod r.rb_cap

(** Truncate ring to last [n] entries (discard older entries). *)
let ring_truncate r n =
  if n >= r.rb_size then ()
  else begin
    (* Shift head back so size = n *)
    r.rb_head <- (r.rb_head - (r.rb_size - n) + r.rb_cap * 2) mod r.rb_cap;
    r.rb_size <- n
  end
...
- [ ] **Step 4: Update EDbg case in eval_expr**
...
- [ ] **Step 3: Add span_of_expr helper to eval.ml**
...
- [ ] **Step 4: Rename eval_expr to eval_expr_inner, add tracing wrapper**
...
Rename the current `apply` body to `apply_inner`. The `eval_match` function already calls `eval_expr` by name — it will naturally call the tracing wrapper.
...
- [ ] **Step 2: Create lib/debug/debug.ml**
...
- [ ] **Step 4: Update test/dune and bin/dune to include march_debug**
...
- [ ] **Step 3: Create lib/debug/trace.ml**
...
|} in
  ...
  (try March_eval.Eval.run_module m' with _ -> ());
  March_debug.Debug.uninstall ();
...
- [ ] **Step 3: Create lib/debug/replay.ml**
...
let replay_from (ctx : Eval.debug_ctx) (new_env : Eval.env) : Eval.value option =
  ...
  | Some frame ->
    ...
    (* Truncate trace at cursor — keep entries older than dc_pos,
       discard the cursor frame and all newer frames *)
    Eval.ring_truncate ctx.dc_trace (ctx.dc_trace.Eval.rb_size - ctx.dc_pos);
    ctx.dc_pos <- 0;
...
- [ ] **Step 1: Create lib/debug/debug_repl.ml**
...
let eval_in_env (env : Eval.env) (tc_env : March_typecheck.Typecheck.tc_env ref)
                (src : string) : unit =
  let lexbuf = Lexing.from_string src in
...
let run_simple_session (ctx : Eval.debug_ctx) (initial_env : Eval.env) : unit =
  ...
  Printf.printf "\n[debug] Breakpoint hit at %s\n%!"
    (match March_debug.Trace.current_frame ctx with
     | None   -> "<unknown>"
     | Some f ->
       let sp = f.tf_span in
    ...
    (match (try Some (input_line stdin) with End_of_file -> None) with
     ...
     | Some line ->
       ...
       (match line with
        ...
        | ":where" | ":w" ->
          Printf.printf "%s\n%!" (March_debug.Trace.show_where ctx)
        | ":stack" | ":sk" ->
          List.iter (fun s -> Printf.printf "%s\n" s)
            (March_debug.Trace.show_stack ctx)
        | ":env" ->
        ...
        | ":replay" | ":r" ->
          (match March_debug.Replay.replay_from ctx !env_ref with
        ...
        | s when String.length s >= 6 && String.sub s 0 6 = ":back " ->
          ...
          let pos = March_debug.Trace.back ctx n in
          Printf.printf "Moved to frame %d.\n%!" pos;
          (match March_debug.Trace.current_frame ctx with
           | None -> ()
           | Some f -> env_ref := f.tf_env)
        | ":back" | ":b" ->
          let pos = March_debug.Trace.back ctx 1 in
          Printf.printf "Moved to frame %d.\n%!" pos;
          (match March_debug.Trace.current_frame ctx with
           | None -> ()
           | Some f -> env_ref := f.tf_env)
        | s when String.length s >= 9 && String.sub s 0 9 = ":forward " ->
          ...
          let pos = March_debug.Trace.forward ctx n in
          Printf.printf "Moved to frame %d.\n%!" pos;
          (match March_debug.Trace.current_frame ctx with
           | None -> ()
           | Some f -> env_ref := f.tf_env)
        | ":forward" | ":f" | ":step" | ":s" ->
          let pos = March_debug.Trace.forward ctx 1 in
          Printf.printf "Moved to frame %d.\n%!" pos;
          (match March_debug.Trace.current_frame ctx with
           | None -> ()
           | Some f -> env_ref := f.tf_env)
        | s when String.length s >= 7 && String.sub s 0 7 = ":trace " ->
          ...
          List.iter (fun s -> Printf.printf "%s\n" s)
            (March_debug.Trace.show_trace ctx n)
        | ":trace" | ":t" ->
          List.iter (fun s -> Printf.printf "%s\n" s)
            (March_debug.Trace.show_trace ctx 10)
...
- [ ] **Step 1: Add --debug flag and debug context installation**
...
- [ ] **Smoke test the example session from the spec**
