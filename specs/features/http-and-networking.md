...
- **Opt-in: event loop** (`runtime/march_http_evloop.c`, `march_evloop_server_listen`). `march_http_server_listen` dispatches here **only** when built with `-DMARCH_HTTP_USE_EVLOOP`, which `bin/main.ml` gates behind the environment variable `MARCH_HTTP_EVLOOP=1` at compile time. It uses **kqueue (macOS/BSD) / epoll (Linux)** with one thread per core, each thread owning a private `SO_REUSEPORT` listener fd and its own event loop that accepts, reads (non-blocking, see `march_http_io.c`), runs the pipeline, and writes the response. It is measurably the cheaper server — ~15–17% less CPU per request, and it scales to concurrency far past any thread count — but its loop threads must not block, so a handler that performs synchronous I/O will stall every other connection on that thread. Prefer it for I/O-light, high-concurrency workloads.

  > This document previously described the event loop as the default. It has not been since it was made opt-in; every March HTTP server built without `MARCH_HTTP_EVLOOP=1` runs the thread pool.
