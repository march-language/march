...
join_points → borrow → perceus → llvm_emit/js_emit) may assume holds, the
...
are atoms without dangling variable references."* This is not merely a
...
`$t27191`) **before** `add(...)`, whose argument list holds only atoms
(`AVar` references to those temps). `--compile` + run: exit 0, prints `31`
(`(10+1) + (10*2)`), matching the interpreter. The identical discipline holds
...
depending on the string it carries:

- **A genuinely named type variable** (`TVar "_NNNN"` or similar) — an
...
Despite the shared representation, `TVar "_"` and a genuinely named `TVar _`
...
`borrow_eligible = true`) precisely because a placeholder is "conservatively
...
- `mono.ml:38` — `is_polymorphic (TVar "_") = false` ("lowering fallback
  placeholder, not a real polymorph") — a genuine `TVar _` IS polymorphic and
...
- `mono.ml:152–158` — when resolving a generic call's substitution, `TVar
  ...
  what prevents infinite mangled-name recursion for a call whose argument
  type could not be determined (`fold_left$List_Int$Int$...` recursing
  forever without this guard, per the module's own comment).
...
`FnLambda`/`FnJoinPoint`-tagged `fn_def` survives Defun to reach
...
| String literal | `"\"..\""` (leading `"`, full quoted text) | `pat_tag_and_subs` (`PatLit(LitString)`) | `llvm_case.ml`: `is_string_case` detects the leading `"`; not a `switch` (no total int encoding) — an if/else chain of `march_string_eq` calls |
...
| Float literal | `"#<hex-float>"` (leading `#`, OCaml `"%h"` exact hex encoding) | `pat_tag_and_subs` (`PatLit(LitFloat)`), **W2 addition** | `llvm_case.ml`: `is_float_case` detects the leading `#`; not a `switch` (no total, exact int encoding of an arbitrary double) — an if/else `fcmp oeq double` chain, decoding the hex string back via `float_of_string` + `Int64.bits_of_float` into the exact LLVM hex-double literal |
...
does not collide with any other tag form"), and it holds transitively for
...
— i.e. the scrutinee (`0x3FF8000000000000` = 1.5's exact IEEE-754 bit
...
Every AST node (`March_ast.Ast.expr`, etc.) carries a `span` for
...
consequence is stated plainly: **every diagnostic emitted by a TIR-consuming
...
`Ast.span` through its own error message precisely because it fires during
...
token for — **no user identifier can ever collide with a synthetic one**,
...
carrying the risk forward from the front-end review
...
| `__try_call` / `__try_call_val` | NOT TIR-synthesized — registered directly as compiler builtins in `typecheck.ml` (arity-1 polymorphic fns) and `eval.ml` (`VBuiltin`); user code calls them explicitly, e.g. `__try_call(fn _ -> ...)` | `lib/tir/borrow.ml`'s `is_try` (via `Tir_names.is_try_call`): recognizes the thunk argument as consumed-and-freed locally, not an escape (see `specs/perceus-invariants.md` §3, rule 2) | Lexable as an ordinary name, but calling a user-defined `__try_call` would simply invoke the builtin (name resolution finds the registered builtin like any other top-level name) — not probed this task |
| `__march_test_N__` | `lib/tir/lower_tests.ml`'s `lower_test_body` via `Tir_names.test_fn_name` (N = a per-module counter) — one synthesized zero-arg fn per `test "..." do ... end` block | Not name-sniffed downstream: the `(fn_name, display_name)` pair is carried structurally in `Tir.tir_module.tm_tests`, consulted by `--test`'s driver-wiring in `lib/tir/llvm_toplevel.ml` and rooted for DCE in `lib/tir/dce.ml` by comparing against the exact registered name, not a re-derived pattern match on the string shape | Yes in principle (double underscore + digits + double underscore is ordinary March syntax), but collision would require guessing the exact per-module counter value |
...
`e5343e84` — current truth, unchanged since Wave 1).** A genuinely
multi-clause function forces `desugar_fn_def`'s general path, which names
its own synthesized scrutinee parameter `__arg0`. A body arm whose own
pattern binds nothing under that name (a bare literal pattern) can still
...
binding without the user ever declaring it:
...
`0`-arm's body plainly observed the live argument value through the
...
type) reproduces this exact order, because `$d < $e < $f <` any legal user
