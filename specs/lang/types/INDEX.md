# Typing corpus index (t01–t88 accept, t01–t81 reject)
...
Exit 0 iff every program behaves as declared (currently 169/169 — 88 accept, 81
...
| `t88_impl_distinct_modules` | **(impl coherence, FQN dispatch Stage 1, accept) two same-short-name types from different modules each `impl` the same interface (§2.3, 2026-07-20)** — `NA.Thing` and `NB.Thing` (nested modules) each declare `impl Speak(Thing)`; coherence keys on the DECLARING MODULE, so the distinct types do NOT overlap. Guards that `register_impl_shape` no longer over-fires on the bare short name. Design `specs/plans/2026-07-20-fqn-impl-dispatch-identity.md` | `--check` exit 0 |
...
**Result: 169 / 169 (88 accept, 81 reject).**
