...
| `accept/t64`–`t68`, `reject/t58`–`t66` | Core March widening slice 7 — linear/affine types, Tasks 2–3 (2026-07-10) | Anchors §2.9 (linearity: marking surfaces, the `env.lin` tracker, transparency). Accept: `linear let` single use (`t64`), `linear` param single use (`t65`), affine type-modifier param dropped (`t66`, also pins the WORKING affine spelling at the time — the param-keyword form was a parse error, finding L1, FIXED 2026-07-11), single arithmetic use of a `linear Int` field (`t67` — REQUIRES the slice-7 L2 fix: constraint discharge now strips `TLin` like `impl_matches_ty` always did; rejected `` does not implement Num `` pre-fix), linear-value send-to-actor as the consuming use (`t68`, corrects the tutorial's "cannot be sent" claim, finding L6). Reject: param drop (`t58`), let drop (`t59`), param double-use (`t60`), match-then-reuse (`t61`), closure capture (`t62`), let-bound field double-access via the `p#data` sentinel (`t63`), affine double-use (`t64`), `always_linear` drop (`t65`), use-after-send (`t66`) — every diagnostic live-pinned; every program uses non-stdlib-colliding names (`Res`/`Tok`/`Cap2`/`StoreRes`, never `Handle`/`Put` — findings L4 and the flat ctor namespace bit the survey probes twice). Findings L1–L6 filed in `specs/todos.md`; survey record in `specs/plans/2026-07-10-widening-linear-types-plan.md` |
| `accept/t81`–`t82`, `reject/t75` | Finding L1 FIX — affine param-keyword + `affine let` grammar (2026-07-11) | `parser.mly`'s `fn_param`, `param`, `block_expr`, and `lambda_stmts` rules each gained an `AFFINE` production mirroring their existing `LINEAR` one — a pure grammar gap (the `Affine` AST constructor and `AFFINE` token already existed; no typechecker change needed). `t81` (param-keyword single use, twin of `t66`), `t82` (`affine let` dropped, previously had NO production at all), `t75` (param-keyword double-use rejects identically to `t64`'s type-modifier spelling) |
...
| `t66_affine_ty_param_drop` | **(T-AffDrop) positive (slice 7, §2.9.2)** — an unused `c : affine Cap2` param typechecks: `check_linear_all_consumed` filters to `Linear` only, so affine values may be silently dropped. Uses the type-modifier spelling (`c : affine T`); the param-keyword form also works since finding L1 was FIXED 2026-07-11 — see `t81` | `--check` exit 0 |
