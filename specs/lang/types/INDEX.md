...
the **F2 FIX** (now Done). `check_pure_module` (`typecheck.ml:6581`) and
`check_deterministic_module` (`:6650`) no longer consult hand-guessed name
lists; both banned sets are now DERIVED from the authoritative
`builtin_cap_table` (`:999`). `pure_banned` = every builtin in the table (all
are effectful) unioned with the incidental non-table impure names
`spawn`/`send`/`exit`. `deterministic_banned` = only the builtins the table
maps to a NONDETERMINISM cap — `IO.Clock`/`IO.Random`, decided by
`is_nondeterministic_cap` (`:6572`) — so `cap deterministic` stays WEAKER than
...
| `t45_cap_pure_file_write` | **`cap pure` (§2.8.11, capabilities widening Task 5 — F2 FIX witness) — the REAL effectful builtin `file_write`** — `check_pure_module` (`typecheck.ml:6581`) now bans every builtin in `builtin_cap_table` (`:999`), which the F2 fix DERIVES `pure_banned` from instead of the pre-fix hand-guessed list that spelled the nonexistent `write_file` and missed `file_write` (mapped `IO.FileWrite`). Type-correct (`file_write : String -> String -> Result(Unit, r)`, signature `Result(Unit, String)`) so the ONLY rejection is the cap ban, not a type mismatch. Accepted (exit 0) pre-fix — this witness was impossible until Task 5 | `` which has side effects `` |
