...
| `t69_sibling_own_ctor` | **sibling-ctor shadowing regression witness (FIXED 2026-07-10)** — sibling modules `A`/`B` each declare `type Foo` with a same-named `Mk` (different payloads), and each uses its OWN `Mk` in its own body; also re-witnesses the §2.5 type-identity design point (`take_a(x : A.Foo)` accepts `B.make()`). This is the ORIGINAL shape of `t35` before its reconciliation: post-`d95fe942`/pre-fix, A's own `Mk(1)` resolved against B's `Mk(String)` (Pass-1 seed order + Pass-2 dedup no-op); `add_ctor` now moves a re-registered entry to the front (declaring-module recency). Unit twin: `test_tc_sibling_ctor_own_module_wins` | run-witnessed: prints `7` |
...
| `t75_refine_precondition_satisfied` | **refinement precondition satisfied at a direct call (slice 12, §2.14)** — `{Int \| _ >= 0}` is transparent to `typecheck.ml` (repr strips it); `take_n(5)` is checked by the separate `lib/refinecheck` pass and provably satisfies the predicate. Reject companion `t71`. Operational witness golden `g46` | `--check` exit 0 |
...
| `t67_letq_last_expr` | **(E-LetQ-Last) (slice 8, §2.10.2)** — a `let?` as the final expression in a block: nothing to return on Ok. Caught natively in infer_expr (the parser folds a trailing `let?` to an empty-body ELetQ) | `cannot be the last expression in a block` |
...
| `t70_letq_type_annotation` | **no type annotation on `let?` (slice 8, §2.10.2)** — `let? x : Int = e` is a parse error. Caught by the GENERIC missing-`=` recovery, NOT the tutorial's promised dedicated message (never implemented — filed); the reject still fires | `expecting \`=\` in the let? binding` |
...
| `t74_offer_wrong_branch_drive` | **(F5 path-dependent offer, reject twin of `accept/t79`, 2026-07-15)** — after per-arm refinement the `:err` arm types the offer channel at `Recv(String, End)`; feeding the received `String` to `int_to_string` is a type error. Pre-fix the first-branch approximation typed it `Int` and this ACCEPTED (a real soundness catch) | `expected \`Int\` but got \`String\`` |
