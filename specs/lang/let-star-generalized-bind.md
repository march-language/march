...
anywhere in the language. There is no higher-kinded machinery, and adding it
...
functions before `let*`, for exactly this reason — nothing about them is
`let*`-specific).
...
1. If the continuation is an empty block (`let*` is the last expression),
   reject immediately — same reasoning as `let?`: there is no way to unify
...
3. `repr result_ty` must reduce to `TCon(head_name, _)` — a concrete type
   ...
   message asking for a concrete type, since there's nothing to look up
...
Because step 5 never hand-constructs `M(...)` itself — it only ever unifies
...
lookup of `<Type>.flat_map`. The continuation is handed to it as a native
