...
**Deferred (Phase 3):** GEP-based vectorizable loop emission with `!llvm.loop.vectorize.enable` metadata and `align 32` allocation (requires `posix_memalign`), plus inlining `map`'s closure call so simple bodies vectorize. The current Phase 2 calls the C runtime's tight loops; LLVM auto-vectorizes the runtime `sum` loops at `-O2`/`-O3` (see correction above), so this is a future enhancement for explicit vectorization hints and for unblocking `map`.
