...
**Deferred (Phase 3):** GEP-based vectorizable loop emission with `!llvm.loop.vectorize.enable` metadata and `align 32` allocation (requires `posix_memalign`) for `sum`/`fold`, and extending Phase 2b's direct-call inlining to `fold` (same non-capturing-closure pattern, same win). The current Phase 2 calls the C runtime's tight loops for `sum`/`fold`; LLVM auto-vectorizes the runtime `sum` loop at `-O2`/`-O3` (see correction above) without needing this, so remaining value here is mostly `fold` and any future ops that don't already have a hand-rolled emitter.
