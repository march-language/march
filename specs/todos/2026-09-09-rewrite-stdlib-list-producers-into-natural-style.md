...
gone: TRMC always runs (no flag or env var turns it off since 2026-09-22), so
natural-style recursion compiles to a loop.
...
| natural style, TRMC off (`--no-trmc`, since removed) | 0.29s |
...
## Resolved: `--no-trmc` is gone (option 1, 2026-09-22)

After this rewrite natural-style recursion is only a loop *because* TRMC makes
it one, so a TRMC-off build would overflow the green-thread stack. Measured
2026-09-09 (natural-style map over `List.range(1, N)`, `--opt 2`): exit 0 up to
20,000 elements, exit 138 with no output from 30,000 with TRMC off. A flag that
looks like a performance knob and aborts on real input should not exist.

The repo owner chose option 1 on 2026-09-22: drop the flag. `MARCH_NO_TRMC`
and the `trmc-suite` CI job went on 2026-09-21
(specs/progress/2026-09-21-ci-shard-test-drop-trmc-suite.md); `--no-trmc`,
`--trmc` and the `Trmc.enabled` ref went on 2026-09-22
(specs/progress/2026-09-22-remove-no-trmc-flag.md). Nothing is left to
settle before the rewrite. The options that were weighed (a debugging-only
flag, or a `--no-trmc` scoped to user code by provenance) are in git history
of this file. The cost accepted: no cheap "is this miscompile TRMC's fault?"
bisection switch.
