...
One `CI` run asks for 32 jobs (23 Linux, 9 macOS), so runs from different PRs
queue behind each other. Keep that in mind before adding a job or a matrix leg:
a new macOS leg costs more queue time than its own run time.
...
test (ubuntu, macos) × codegen | refinecheck | compiler | rest
two-node (ubuntu)
bench-gate (ubuntu)
...
| `test (<os>, codegen)` | `dune build @test/runtest-run_codegen`: the LLVM codegen suite (`test/test_codegen.ml` and friends), including native compile-and-run cases. | A codegen or runtime regression; reproduce with `scripts/run-tests.sh codegen`. |
| `test (<os>, refinecheck)` | `@test/runtest-test_refinecheck`: the z3-backed refinement checker corpus. | `scripts/run-tests.sh refinecheck` (needs z3 on PATH). |
| `test (<os>, compiler)` | `@test/runtest-run_compiler`: frontend, typecheck, capabilities, CAS, CLI behaviour. | `scripts/run-tests.sh compiler`. |
| `test (<os>, rest)` | `MARCH_CI_RUNTEST_SPLIT=1 dune runtest`: everything else under `dune runtest` (eval, stdlib, JIT, LSP, forge, the native golden rules, snapshots, C unit tests). The env var drops exactly the three suites above; see the comment over the test runners in `test/dune`. | `dune runtest --root .` locally (without the env var, which runs the other three as well). |
