...
let rec infer_expr env (e : Ast.expr) : ty =
  let result =
    ...
    (* `Node.send(peer, to, msg)`: an ordinary stdlib function, typed and
       ...
       NodeQueue: its message is the third argument too.  `Node.accepts(d,
       fn (_ : T) -> ())` is the receiver's half (generated by `@[remote]`,
       Desugar_remote): the tag is minted from the witness's parameter type. *)
    | Ast.EApp ((Ast.EVar { txt = ("Node.send" | "Node.enqueue" | "Node.accepts") as callee; _ }) as fv, args, sp)
...
let check_module_core ?(errors = Err.create ()) ?seed_env (m : Ast.module_)
  ...
  check_node_send_sites ~entry:m.Ast.mod_name.txt final_env;
