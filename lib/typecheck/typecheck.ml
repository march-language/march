...
(* Binding a value name SHADOWS any same-named module fn for the direct-call
   ...
   accumulates across modules, so before this removal a stdlib param like
   fold_left's `f : b -> a -> b` collided with ANY user/other-module `fn f`
   of a different arity — the false arity error was reported at the STDLIB
   span (silently filtered by bin/main.ml's is_user_file), the call was
   "recovered" as a PARTIAL application (peel), the enclosing fn's recursive
   call then hit an occurs failure (acc := elem -> acc), the type var was
   linked to TError, and the poisoned type_map lowered fold_left with a
...
let check_fn env (def : Ast.fn_def) fn_span : scheme =
  ...
  (* The self-bind above cleared any fn_arities entry for this name (shadow
     ...
     arity).  Re-register the CURRENT def's own arity so recursive calls in
     the body are still arity-checked, against the right arity either way. *)
  let env_rec =
    ...
    let pre_env = List.fold_left (fun e d ->
        ...
        | Ast.DFn (def, _) ->
          ...
          (* bind_var FIRST (it clears any shadowed fn_arities entry), then
             register this fn's own arity so the entry survives. *)
