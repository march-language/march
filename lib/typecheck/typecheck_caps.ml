...
(** A message type's SCHEMA HASH (build step 9): a digest of its structure,
    not its name -- a record's fields (name and type) in declaration order, a
    variant's constructors (sorted by name: the JSON tag is the constructor,
    so order does not reach the wire) with their argument types; a declared
    type they mention is expanded the same way (a back-reference for a
    recursive one), a builtin is its name. Two builds agree on the hash iff
    `derive Json` writes the same shape, so a receiver can tell a same-named
    type of another shape. [name] is the declaration as [json_dispatch_head]
    resolves it. *)
let schema_hash (env : env) (name : string) : string =
  let short n = match String.rindex_opt n '.' with
    | Some i -> String.sub n (i + 1) (String.length n - i - 1) | None -> n in
  let find_record n =
    match StrMap.find_opt n env.records with
    | Some r -> Some r
    | None -> StrMap.find_opt (short n) env.records
  in
  let ctors_of n =
    let s = short n in
    StrMap.fold (fun c infos acc ->
        match List.find_opt (fun ci -> ci.ci_type = s) infos with
        | Some ci -> (c, ci.ci_arg_tys) :: acc
        | None -> acc) env.ctors []
    |> List.sort (fun (a, _) (b, _) -> compare a b)
  in
  let rec key seen (t : Ast.ty) =
    match t with
    | Ast.TyCon (c, args) ->
      let base = if args = [] then "" else "(" ^ String.concat "," (List.map (key seen) args) ^ ")" in
      if List.mem c.txt seen then "@" ^ short c.txt ^ base
      else (match def (c.txt :: seen) c.txt with
          | Some d -> d ^ base
          | None -> short c.txt ^ base)
    | Ast.TyVar v -> "'" ^ v.txt
    | Ast.TyTuple ts -> "(" ^ String.concat "," (List.map (key seen) ts) ^ ")"
    | Ast.TyArrow (a, b) -> "(" ^ key seen a ^ "->" ^ key seen b ^ ")"
    | Ast.TyRecord fs -> "{" ^ String.concat "," (List.map (fun ((f : Ast.name), t) -> f.txt ^ ":" ^ key seen t) fs) ^ "}"
    | Ast.TyLinear (_, t) -> key seen t
    | _ -> "?"
  and def seen n =
    match find_record n with
    | Some (_, fields) -> Some ("{" ^ String.concat "," (List.map (fun (f, t) -> f ^ ":" ^ key seen t) fields) ^ "}")
    | None ->
      (match ctors_of n with
       | [] -> None
       | cs -> Some ("<" ^ String.concat "|" (List.map (fun (c, ts) -> c ^ "(" ^ String.concat "," (List.map (key seen) ts) ^ ")") cs) ^ ">"))
  in
  let body = match def [ name; short name ] name with Some d -> d | None -> short name in
...
(** The typed remote send, the call-site half (stdlib/node.march).  For every
    ...
    naming the site. *)
let check_node_send_sites ~(entry : string) (env : env) : unit =
  ...
  let qualified_name n =
    ...
    let chosen =
      ...
      | [] -> n
    in
    (* The entry module's own name is unwrapped, as it is for a nested type
       (`Msgs.Ping`, not `App.Msgs.Ping`): a type declared at the entry
       module's top level is registered under "App.Note" too, and minting
       that made the tag depend on what each PROGRAM's entry module is called
       -- two nodes built from different entry files could never agree on it. *)
    let prefix = entry ^ "." in
    let le = String.length prefix and lc = String.length chosen in
    if entry <> "" && lc > le && String.sub chosen 0 le = prefix
    then String.sub chosen le (lc - le)
    else chosen
  in
  ...
  in
  (* The message type: the third parameter of `Node.send` / `Node.enqueue`;
     for a witness form (`Node.accepts(d, witness)` and the schema forms of
     build step 9) the witness function's parameter. *)
  let message_ty callee f_ty =
    let witness_param w = match repr w with TArrow (m, _) -> Some m | _ -> None in
    match March_ast.Json_dispatch.witness_arg callee with
    | Some 0 -> (match repr f_ty with TArrow (w, _) -> witness_param w | _ -> None)
    | Some _ ->
      (match repr f_ty with
       | TArrow (_, r1) ->
         (match repr r1 with
          | TArrow (w, _) -> witness_param w
          | _ -> None)
       | _ -> None)
    | None -> third_param f_ty
  in
  List.iter (fun (sp, raw_callee, f_ty) ->
      (* A witness site is generated by `@[remote]`: name what the user
         wrote, the actor's remote handler, not the intrinsic. *)
      let callee =
        if March_ast.Json_dispatch.witness_arg raw_callee <> None && raw_callee <> "Node.schema_of"
        then "@[remote] dispatch" else raw_callee in
      match message_ty raw_callee f_ty with
      | None ->
      ...
      | Some msg_ty ->
        (match json_dispatch_head env msg_ty with
         ...
         | Some name ->
           let s = short name in
           if has_json_codec s then begin
