...
let check_node_send_sites ~(entry : string) (env : env) : unit =
  ...
  (* The message type: the third parameter of `Node.send` / `Node.enqueue`;
     for `Node.accepts(d, witness)` the witness function's parameter. *)
  let message_ty callee f_ty =
    if callee = "Node.accepts" then
      match repr f_ty with
      | TArrow (_, r1) ->
        (match repr r1 with
         | TArrow (w, _) -> (match repr w with TArrow (m, _) -> Some m | _ -> None)
         | _ -> None)
      | _ -> None
    else third_param f_ty
  in
  List.iter (fun (sp, callee, f_ty) ->
      (* A `Node.accepts` site is generated by `@[remote]`: name what the user
         wrote, the actor's remote handler, not the intrinsic. *)
      let callee = if callee = "Node.accepts" then "@[remote] dispatch" else callee in
      match message_ty (if callee = "@[remote] dispatch" then "Node.accepts" else callee) f_ty with
      | None ->
