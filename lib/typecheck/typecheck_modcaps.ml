...
(* What remains SYNTACTICALLY banned among qualified stdlib names: only the
   ones with no refinement contract to consult.  Empty since 2026-09-13.

   `Array.get`/`Array.set`/`Array.pop` were the last three (Group B): the
   2026-08-05 feasibility gate found their contract could not be discharged,
   because `Array.length` reads a scalar CONSTRUCTOR FIELD and call-site
   reflection erased every such field.  That erasure is fixed
   (specs/progress/2026-09-13-measure-over-scalar-ctor-field.md), the three
   carry bounds preconditions over the `Array.length` measure, and they moved
   to [panic_surface_contracted] below after a stdlib + ecosystem
   `--refine-report` sweep (specs/progress/2026-09-13-array-bounds-contracts.md).

   Kept as a set, not deleted: a future stdlib name that panics with no
   contract belongs here.  Ban-list audit 2026-08-05
...
let panic_surface_contracted : StringSet.t = StringSet.of_list [
  ...
  (* Bounds contracts since 2026-09-13: `Array.length` is a `@[measure]` over
     `PVec`'s count field, which call-site reflection now places concretely
     (specs/progress/2026-09-13-measure-over-scalar-ctor-field.md), so these
     three are proof-checked rather than banned by name. *)
