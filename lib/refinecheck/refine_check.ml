          ...
          Err.warning errctx ~span:fd.A.fn_name.A.span
            (Printf.sprintf
               "@[measure] `%s` reads a constructor field that is not itself a \
                data type; a call site reflects such a field as an unknown \
                unless it is an Int literal, so refinements using it are \
                proved or refuted only for literal payloads."
               name))
      mfns
  end;
