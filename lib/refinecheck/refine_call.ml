...
let check_call (cx : call_ctx) ~span ~(callee : string) ?(subject = Argument)
  ...
  | Some self_actual ->
    ...
    and reflect_field a = function
      ...
      (* An Int LITERAL payload is reflected as itself: a set-valued measure
         reads payloads into sets (`Var(x) -> singleton(x)`), and erasing a
         literal would leave `free_vars(Lam(1, Var(1))) == empty` unprovable.
         Sound either way (a literal is more constrained than a fresh
         constant); a VARIABLE payload stays a fresh constant, as before. *)
      | Smt.SInt when (match a with A.ELit (A.LitInt _, _) -> true | _ -> false) ->
        (match a with A.ELit (A.LitInt n, _) -> Some (Smt.IntLit n) | _ -> None)
      | sort ->
