...
let emit_int_cmp ~emit_atom ctx (f : Tir.var) (a : Tir.atom) (b : Tir.atom)
  : string * string =
    ...
    if is_string_eq then begin
      ...
      (f.Tir.v_name = "==" || f.Tir.v_name = "!=")
      && (Kind.unboxed_of_llvm_ty ctx.Llvm_ctx.k_table ty_a <> None || Kind.unboxed_of_llvm_ty ctx.Llvm_ctx.k_table ty_b <> None)
    then begin
      ...
      let sty = match Kind.unboxed_of_llvm_ty ctx.Llvm_ctx.k_table ty_a with
        | Some _ -> ty_a | None -> ty_b in
      let fields = match Kind.unboxed_of_llvm_ty ctx.Llvm_ctx.k_table sty with
        | Some (_, _, fs) -> fs
