...
let check_slot_ty (ctx : Llvm_ctx.ctx) (where : string) (ty_str : string) =
  match Kind.unboxed_of_llvm_ty ctx.Llvm_ctx.k_table ty_str with
  | None -> ()
  | Some (tname, _, _) ->
...
let emit_store_field ctx obj_val i ty_str val_str =
  check_slot_ty ctx "emit_store_field" ty_str;
...
let emit_load_field ctx obj_val i ty_str =
  check_slot_ty ctx "emit_load_field" ty_str;
  let offset = 16 + i * 8 in
  let fp = Llvm_ctx.fresh ctx "fp" in
