...
(* ── ADT structural equality generation ───────────────────────────────
   ...
   here for the same reason as above.  [ensure_adt_eq_fn]'s only caller in
   this file was the `==`/`!=` arm body, now in [Llvm_emit_arith], which
   reaches for [Llvm_eq] directly. *)
