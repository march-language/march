  ...
  | Tir.TCon (type_name, ty_args) ->
    ...
    if Hashtbl.mem ctx.Llvm_ctx.emitted_eq_fns fn_name then Some fn_name
      ...
      let newtype_payload_opt =
        match Kind.repr_of ctx.Llvm_ctx.k_table ty with
        | Kind.Newtype raw_payload ->
      ...
      let niche_payload_opt =
        if Kind.is_niche_shaped ctx.Llvm_ctx.k_table type_name then
          match ty_args with
          | [p] when Kind.niche_payload_ok ctx.Llvm_ctx.k_table p -> Some p
      ...
      | Some payload_ty ->
        ...
        (match payload_ty with
         ...
         | _ when Kind.payload_needs_tag ctx.Llvm_ctx.k_table payload_ty ->
