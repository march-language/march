(* Capability-bearing runtime entry points.
   ...
   Generated by joining [builtin_cap_table] (lib/typecheck/typecheck_builtins.ml)
   ...
   Every key is a `march_`-prefixed runtime symbol.  An unprefixed key would
   also match any user or foreign symbol of the same spelling: until
   2026-09-25 `dns_resolve`'s C function was unprefixed and keyed here as
   "dns_resolve", so the binary audit read any symbol spelled `dns_resolve`
   as an IO.Network witness.
...
let table : (string * string) list = [
  ...
  (* IO.Network *)
  ("march_dns_resolve",            "IO.Network");
  (* IO.NetListen *)
  ("march_tcp_listen",             "IO.NetListen");
  ("march_tcp_accept",             "IO.NetListen");
  ("march_tcp_accept_timeout",     "IO.NetListen");
  ...
  ("march_unix_time_ms",           "IO.Clock");
  ("march_uuid_v7",                "IO.Clock");
  (* IO.Random *)
...
(* March-name builtins from builtin_cap_table with NO compiled lowering.
   ...
   and on any entry listed here that HAS gained a lowering.
   - task_spawn_link: interpreter-only.  A compiled call is rejected at
     lowering with a positioned diagnostic
     (Lower_expr.interpreter_only_builtin_reasons): the compiled task runtime
     has no task/actor link, so there is nothing to observe in a binary.
   - get_work_pool:   lowers to a null pool-handle atom (llvm_emit.ml's
     emit_atom), not a call — no symbol is referenced.
   unix_time_ms and uuid_v7 were listed here until 2026-09-25 as having no
   compiled lowering.  Both did compile — unix_time_ms through its
   march_unix_time_ms row (since 2026-08-22), uuid_v7 through mangle_extern's
   identity fallthrough to an unprefixed C `uuid_v7` — so listing them here
   kept IO.Clock out of every binary that used them.  Both are rows in
   [table] now. *)
let uncompiled_builtins : string list =
  [ "task_spawn_link"; "get_work_pool" ]
