...
let task_builtins : env =
  ...
  ; ("worker", VBuiltin ("worker", function
      ...
      | [VString name] ->
        VRecord [("actor", VString name); ("restart", VAtom "permanent")]
      (* Two-arg form: if second atom is a restart policy use it; otherwise treat as registered name *)
      | [VCon (name, []); VAtom kw] ->
        let restart_policies = ["permanent"; "temporary"; "transient"] in
        if List.mem kw restart_policies then
          VRecord [("actor", VString name); ("restart", VAtom kw)]
        else
          VRecord [("actor", VString name); ("restart", VAtom "permanent"); ("name", VAtom kw)]
      | [VString name; VAtom kw] ->
        let restart_policies = ["permanent"; "temporary"; "transient"] in
        if List.mem kw restart_policies then
          VRecord [("actor", VString name); ("restart", VAtom kw)]
        else
