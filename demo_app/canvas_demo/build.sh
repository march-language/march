#!/usr/bin/env bash
# Build canvas_demo to JS by invoking the dev march compiler directly.
# canvas_demo has no external forge dependencies (just stdlib), so it
...
echo "Compiling canvas_demo → canvas_demo.mjs..."
MARCH_STDLIB="$MARCH_STDLIB" "$MARCH_EXE" --target js \
  -o "$DEMO/canvas_demo.mjs" "$DEMO/lib/canvas_demo.march"
...
echo "Done! Open demo_app/canvas_demo/index.html in a browser."
