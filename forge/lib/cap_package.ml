...
(* ── dependency capability lockfile ────────────────────────────────── *)

(* Recorded rather than fetched from the registry: the registry does not store
   capability sets yet, and this check does not need it to.  The baseline is a
   local record of what each dependency needed when it was last reviewed, so a
   later resolve that WIDENS one is visible before it is built — which is the
   xz / event-stream moment. *)

let baseline_path root = Filename.concat root ".forge/cap-baseline"

let load_baseline root : (string * string list) list =
  let p = baseline_path root in
...
let save_baseline root (entries : (string * string list) list) =
  let dir = Filename.concat root ".forge" in
  (try Project.mkdir_p dir with Sys_error _ -> ());
  let oc = open_out (baseline_path root) in
  output_string oc
    "# Reviewed capability set per dependency. `forge cap deps --check`\n\
     # fails when a dependency needs MORE than it did here; re-record with\n\
     # `forge cap deps --accept` after reviewing the change.\n";
  List.iter
    (fun (name, caps) ->
      Printf.fprintf oc "%s %s\n" name (String.concat "," caps))
