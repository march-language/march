...
(** B7 regression: Perceus wraps a borrow-induced post-call DecRC around a
    ...
    FLIPPED 2026-09-20 (specs/progress/2026-09-20-mutual-tco-safety.md): the
    live-path DecRC this test demanded is a use-after-free.  [prefix] is the
...
(** Companion to B7 above, written while the filter refused B7's shape (it
    ...
    - a `Cons(x, rest)` walk that hands the tail to the other member: every
      ...
      nothing in the chain releases a forwarded, non-dup-bound argument.  This
      is the second half of test/native/mutual_tco_forwarded_arg.march, whose
      live-allocation check pins that it does not leak either. *)
let test_mutual_tco_safe_group_still_flattened () =
  let ir_ints = emit_mutual_tco_ir {|mod Test do
  needs IO.Console
    @[no_warn_recursion]
    fn ev(n : Int) : Bool do
      if n == 0 do true else od(n - 1) end
    end
    @[no_warn_recursion]
    fn od(n : Int) : Bool do
      if n == 0 do false else ev(n - 1) end
    end
    fn main(_cap_console : Cap(IO.Console)) : Unit do println(to_string(ev(1000000))) end
  end|} in
  Alcotest.(check bool) "safe mutual group (ints): mutual_loop emitted" true
    (ir_contains ir_ints "mutual_loop");
  Alcotest.(check bool) "safe mutual group (ints): combined fn declared" true
    (ir_contains ir_ints "__mutco_");
  let ir_list = emit_mutual_tco_ir {|mod Test do
  needs IO.Console
    @[no_warn_recursion]
    fn walk(xs : List(String), seen : Int) : Int do
      match xs do
        Nil -> seen
    ...
    @[no_warn_recursion]
    fn step(x : String, rest : List(String), seen : Int) : Int do
      if String.starts_with(x, "no") do walk(rest, seen + 1) else walk(rest, seen) end
    end
    fn main(_cap_console : Cap(IO.Console)) : Unit do println(int_to_string(walk(["no-a", "b"], 0))) end
  end|} in
  Alcotest.(check bool) "safe mutual group (owned list walk): mutual_loop emitted" true
    (ir_contains ir_list "mutual_loop");
  Alcotest.(check bool) "safe mutual group (owned list walk): back-edge branch emitted" true
    (ir_contains ir_list "br label %mutual_loop")
...
declare ptr  @march_try_call_val(ptr %thunk)
...
declare ptr  @march_value_to_string_repr(ptr %v)
...
declare ptr  @march_ed25519_seed_keypair(ptr %seed)
declare ptr  @march_ed25519_sign(ptr %sk, ptr %msg)
declare i64  @march_ed25519_verify(ptr %pk, ptr %msg, ptr %sig)
declare ptr  @march_x25519(ptr %k, ptr %u)
...
declare ptr  @march_uuid_v7()
declare ptr  @march_uuid_v7_at(i64 %unix_ms)
...
declare ptr  @march_logger_add_field(ptr %key, ptr %value)
declare i64  @march_logger_field_count()
declare ptr  @march_logger_get_fields()
declare ptr  @march_logger_pop_to_depth(i64 %depth)
declare ptr  @march_logger_dispatch(ptr %level, ptr %msg, ptr %source, ptr %fields)
declare ptr  @march_logger_register_appender(ptr %name, ptr %cb)
declare ptr  @march_logger_remove_appender(ptr %name)
declare ptr  @march_logger_clear_appenders()
declare ptr  @march_logger_appender_names()
declare ptr  @march_logger_set_module_level(ptr %module, i64 %level)
declare ptr  @march_logger_clear_module_level(ptr %module)
declare i64  @march_logger_module_level(ptr %module)
...
declare ptr  @march_tcp_accept_timeout(i64 %fd, i64 %timeout_ms)
...
declare void @march_tcp_shutdown(i64 %fd)
declare void @march_sleep_ms(i64 %ms)
...
declare ptr  @march_dns_resolve(ptr %host)
...
declare ptr    @native_float_arr_sort(ptr %arr)
...
declare i64  @march_http_fetch_available()
declare ptr  @march_http_fetch(ptr %method, ptr %url, ptr %headers, ptr %body)
; CSV builtins
...
declare void @march_epoch_hold()
declare void @march_epoch_release()
declare i64  @march_epoch_draining()
