...
(* Returns (exit_code, captured_output) rather than failing, so each platform
   can assert the behaviour that is correct FOR IT: macOS embeds a profile,
...
let test_cap_sandbox_profile_is_module_specific () =
  ...
  if not is_macos then begin
    (* Linux: self-sandboxing needs in-process seccomp-bpf, which is not
       implemented. The compiler must REJECT the flag rather than emit a
       binary that exits 70 on first run — a silent trap for anyone who
       passes --cap-sandbox and only finds out in production. *)
    ...
    Alcotest.(check bool) "the rejection names the external alternative" true
      (let re = Str.regexp_string "forge cap run" in
       try ignore (Str.search_forward re out_p 0); true with Not_found -> false)
  end
  else begin
    if rc_p <> 0 then
